...
| **Conformance suite v0.1.0 — vectors + runner (2026-06-17)** | a black-box suite a second impl must pass, derived from the real code | `Proofs/lineage-groups/crates/conformance`: `emit-vectors` runs the real `lineage-core`/`lineage-history` API to write language-neutral JSON; `run-vectors` re-feeds + diffs. **38 pass / 0 fail** across cat1 derivations, cat2 signing (+ one-bit-flip reject), cat3+4 fold/thresholds (incl. E2.10 one-lineage quorum reject), cat5 revocation mechanics, **cat5b revoke-AUTHORITY** (4 vectors: 2-lineage accept; under-threshold/non-admin/one-lineage-multi-device reject — derived from the green-real `gov::meets_threshold_by_lineage` k-of-n Ed25519), cat6 reconcile C1–C10, manifest integrity. Must-reject teeth verified. The revoke-authority PLACEHOLDER is now a real vector (signature mechanism is green-real; only over-the-wire authority distribution + co-sign ordering remain Workstream C). cats 7/8/9 not-yet-emitted. **⚠️ Surfaced: CROFT-PROTOCOL §2 domain-tagged genesis/topic pre-images diverge from `lineage-core`'s plain sha256 + `"lineage-topic-v1"` topic tag — a real reconciliation item.** | green (green-real/green-model rows; design rows deferred) |
