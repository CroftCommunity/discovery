...
| **Workstream C part 2: MLS key-distribution over the wire (2026-06-17)** | distribute the group key over the transport, not as agreed state | `mls-welcome-over-iroh` (real openmls 0.8.1 + iroh =1.0.0 coexist): a real 796-byte openmls **Welcome** travels a real iroh connection (founder→joiner, homed on a real relay); the joiner runs `join_from_welcome` and derives the **same 32-byte MLS exporter secret** as the founder (both epoch 1, member_count 2). The faithful crate's "registry as agreed state" honesty boundary is closed for key distribution. `experiments/iroh/relay-lab-runs/C-mls-welcome-2026-06-17/` | green-real (transport; loopback single-process — cross-host is a trivial follow-on). With part 1, both Workstream C transport boundaries addressed. |
