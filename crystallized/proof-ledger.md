...
| E2.16 | tier degradation visibility (forward-send works + stale surfaced without superpeer) | **green-model (2026-06-16)** — `lineage-group-model` E2.16a/b/c on the freshness model (`core/freshness.ts`). (a) availability: a peer with no superpeer/peers still applies local ops (seq advances). (b) no-false-current: silence → "unverified", a later seen tip → "behind", caught-up+heard → "current"; a peer advancing its own head while hearing no one stays "unverified". (c) tiers degrade visibly: interactive (15s) flips to unverified fast, quiet-large (6h) holds within its horizon, broadcast judges by position — and NO tier shows "current" while behind+stale. Multi-device tier now complete incl. tier visibility. |
...
| **T2g/MD-G3: drop-a-device mid-run (2026-06-16)** | killing the bootstrap relayer mid-run leaves the survivors converged | node-1 + node-4(Mac) bootstrapped only via node-2; node-2 killed ~14s in; both survivors hold each other's branch (transitive mesh survives the relayer's death). Pairs with B-gossip's post-kill new-delivery result. | green-real (transport) |
| **T2g/MD-G4: multi-device fold over transport (2026-06-16)** | many devices of one user fold to one actor over the wire; a second user is a second actor | topic now derived from the **group** (shared); each branch carries `device_did`+`lineage_id`; receivers `fold_by_lineage`. Group `g1`: alice.laptop(node-1)+alice.phone(NAT Mac)+bob.phone(node-2) → all three nodes `folded_actors=2 actors={alice:[laptop,phone],bob:[phone]} rejected=2(tampered)`. Transport form of E2.9/C4 `fold_by_lineage`. | green-real (transport, structural half) |
| **T2g/MD-G5: revoked device can't rejoin over transport (2026-06-16)** | a MAC'd revoke marker bars a device from the accepted set; pre-revoke history is retained | group `g2`, alice.laptop revokes carol.tablet. Witness (Mac/bob) **retains** carol's pre-revoke branch + marks `revoked={carol.tablet}` (history not clawed back); revoker first-hears carol post-revoke → `REJECT (revoked)` (kept out); target `received REVOKE of SELF — cannot re-enter`. Revoke integrity = sha-256 MAC (Ed25519 authority green-real in E2.11). | green-real (transport, structural half) |
| **FAITHFUL: real signed message + authority over the wire (2026-06-16)** | the REAL Ed25519-signed `lineage_history::Message` travels live iroh and is verified for **signature AND standing** on receipt — closes the honesty boundary all hash-chain transport rows carried | `altdrive-spike-faithful-sync` path-deps the real `lineage-core`/`lineage-history` (stable ed25519-dalek 2.2.0) alongside iroh rc.1 — they compose. Runs the real `backfill_import` on the wire. Both joiners (node-2 + NAT Mac) agree: HONEST(member)→ACCEPT; FORGED→REJECT BadSignature; **NONMEMBER (valid sig, no standing)→REJECT UnauthorizedAuthor** — the attack a hash chain cannot catch. Remaining: MLS key-distribution + threshold revoke-authority not over wire (see `thinking/revocation-authority.md`). | green-real (transport, FULL — authorship+standing) |
| **AR-4: transport metadata-leak bound (2026-06-16)** | what a relay/on-path observer can infer | content opaque (encrypted QUIC); relay is topic-agnostic (routes by EndpointId, doesn't learn the lineage topic); relay DOES see EndpointId pairs + timing + volume + egress IP (classic traffic analysis). **Design input: topic=sha256(lineage id) ⇒ lineage ids must be high-entropy/salted, not guessable handles.** | characterized (quantifying capture is a follow-on) |
| **E10: RoQ media datagram-CC bound — the C1 unknown (2026-06-17)** | does iroh's QUIC congestion control fight a media bitrate estimator on the datagram path | Synthetic CBR (64 kbps/20 ms) over `conn.send_datagram` (negotiated 1162 B), node-1→node-2 through the E6 netem rig. **Loss/delay TRANSPARENT** (5 %→4.6 %, 30 %→30.9 % pass through as gaps; RTT flat ~2 ms, tracks delay exactly 100→102 ms; sub-ms jitter; no HOL) — a media estimator gets accurate loss+RTT+jitter, audio holds to 30 % loss with *visible* degradation. **Over-cap source BUFFERBLOATS** (40 kbit cap → RTT 537→8829 ms; receiver gets a contiguous *zero-gap* delayed prefix at link rate; `send_datagram` never errors — drops oldest silently). **Design consequence:** the media estimator MUST be authoritative and back off on the path-RTT trend (C1 solution point 2 confirmed). **E10c/TC-CC2:** a delay-based AIMD estimator on a mid-call bandwidth drop backs off 64→8 kbps in <1s and bounds RTT to ~1s with a continuous lossless stream (fixed diverges past 7s) — the signal is actionable; it rescues the degradation case, not the join-already-saturated case. **TC-CC3:** a bulk transfer on the SAME connection STARVES the media (RTT 50ms→9.5s) → media + bulk MUST be on separate flows (C1 point 3 now mandatory). `experiments/iroh/relay-lab-runs/E10-roq-netem-2026-06-17/` | characterized (C1 resolved incl. estimator + isolation; mesh-vs-meer + raw-UDP baseline follow-ons) |
| **E12: blind media-meer SFrame-over-MLS — the C3 keying unknown (2026-06-17)** | media E2EE keyed off a real MLS group, forwarded by a meer that holds no key | Real openmls 0.8.1 group (via `lineage-mls`); per-sender SFrame key = HKDF(MLS exporter secret, epoch‖leaf); ChaCha20-Poly1305 frames. **TC-KEY1** distinct per-sender keys + non-member can't derive (media `UnauthorizedAuthor`); **TC-KEY2** 90/90 frames decrypt out-of-order under 10 % loss + reorder, 90/90 replays rejected; **TC-KEY3** removal advances the epoch + rotates the secret → revoked sender's later frames rejected (media **MD-G5**), pre-revoke history retained, group works post-removal; **TC-KEY4** blind SFU routes 92/92 frames from clear headers, recovers **0** plaintext. `experiments/iroh/relay-lab-runs/E12-sframe-mls-2026-06-17/` | green-real (synthetic frames/modeled SFrame header; transport-carried = E10 rig + this, a follow-on) |
| **meer P0+P1: always-on blind Tier-0 mirror (2026-06-17)** | a blind always-on superpeer serves offline state holding no payload key; delegation stays materially reversible | `relay-loadtest meer`/`meer-member` over live iroh (meer node-1, members node-2). Member publishes ChaCha20-Poly1305 blobs stored by sha256; an offline member syncs through the meer + decrypts locally → **converges 5/5**; meer stats prove blindness (`meer_payload_keys_held=0`, sees only §3b metadata); export→import into a replacement meer B → re-homed member converges 5/5 (**state portability** anti-entrenchment guard); admission allowlist denies a non-listed peer (`not admitted code 1`). Converts E3.4's modeled blind broker into a running binary. `experiments/iroh/relay-lab-runs/E9-meer-tier0-2026-06-17/` | green-real (spike harness; production-TDD meer crate = Workstream B; P2–P6 remain) |
| **E11: MoQ broadcast — lazy fan-out + blind relay + metadata admission (2026-06-17)** | the Croft-novel broadcast-relay properties (the abuse lever) | `crates/moq-lazy-spike` (relay logic, deterministic): **lazy** (0 publisher egress across 100 ticks with no subscriber; produces when watched); **fan-out linear in N** (0→0, 1→10, 3→30, 10→100 for a 10-frame track); **blind** (relay payload_keys_held=0, subscribers decrypt locally); **metadata admission** (max-audience cap refuses 3 of 8, members-only refuses 1 non-member — reading zero frame bytes = the Rave-trap answer: scale + peer restriction, not content inspection). `experiments/iroh/relay-lab-runs/E11-moq-lazy-2026-06-17/` | characterized (relay logic green; full moq-rs/GStreamer/WebTransport + transport-carried = meer P5) |
| **Workstream C part 1: real revoke-authority signature over the wire (2026-06-17)** | retire the MD-G5 transport MAC — carry a real k-of-n threshold signature, not a MAC | The faithful crate (`altdrive-spike-faithful-sync`) now carries a real `gov::SignedOp` (k-of-n Ed25519) over live iroh-gossip via a tagged `WireItem`; the joiner (node-2) verifies it with the green-real `meets_threshold_by_lineage`. **REVOKE-AUTHORIZED** (alice+bob = 2 admin lineages ≥ 2) → ACCEPT; **REVOKE-UNDERTHRESHOLD** (alice only) → REJECT — alongside HONEST/FORGED/NONMEMBER. MD-G5 transport MAC retired; only the co-sign-vs-vote ordering decision remains. `experiments/iroh/relay-lab-runs/C-faithful-revoke-2026-06-17/` | green-real (transport). Part 2 (MLS key-dist over the wire, messaging crate) remains — E12 proved the keying in-process. |
| **Workstream C part 2 + integration: MLS key-dist AND standing over the wire (2026-06-17)** | distribute BOTH the group key and the membership/standing over the transport, not as agreed state | `mls-welcome-over-iroh` (real openmls 0.8.1 + iroh =1.0.0 coexist): a real openmls **Welcome** (1466 B, leaves carrying T1 lineage claims) travels a real iroh connection; the joiner runs `join_from_welcome` and derives (1) the **same 32-byte MLS exporter secret** as the founder AND (2) the **same lineage-folded standing** — alice's 2 devices fold to 1 actor (E2.9/E2.10), bob a 2nd — entirely from the wire-delivered Welcome. **The integration (E12 + C-mls-welcome + faithful):** the verifying-key/standing registry is now sourced from the real wire-distributed MLS group, not modeled. Closes the "registry as agreed state" boundary. `experiments/iroh/relay-lab-runs/C-mls-welcome-2026-06-17/` | green-real (transport; loopback — the iroh transport is already proven cross-host by E10/meer/faithful). With part 1, both Workstream C transport boundaries addressed + bound to standing. |
| **Conformance suite v0.1.0 — vectors + runner (2026-06-17)** | a black-box suite a second impl must pass, derived from the real code | `Proofs/lineage-groups/crates/conformance`: `emit-vectors` runs the real `lineage-core`/`lineage-history` API to write language-neutral JSON; `run-vectors` re-feeds + diffs. **66 pass / 0 fail** across cat1 derivations (incl. the §2 **tagged wire derivations** — lineage/group genesis + topic — now canonical in `lineage-core::ids`), cat2 signing (+ one-bit-flip reject), cat3+4 fold/thresholds (incl. E2.10 one-lineage quorum reject), cat5 revocation mechanics, **cat5b revoke-AUTHORITY** (k-of-n Ed25519, lineage-counted), cat6 reconcile C1–C10, **cat7 adversarial AR-1/2/3/6** (Sybil no-standing, sequencer reorder/drop/inject, replay/double-count, backfill bounded-cost — AR-4 metadata-leak + AR-5 tree-scaling honestly deferred as characterizations), **cat8 visibility V1–V9+S2** and **cat9 freshness E2.16** (emitted from the runnable TS `lineage-group-model`; TS is the authoritative runner, the Rust suite structure-validates), manifest integrity. Must-reject teeth verified throughout. **Spec-vs-code derivation discrepancy RESOLVED** (the §2 tagged pre-images are now canonical in lineage-core; `lineage-iroh` topic uses the spec `croft-group-topic:` form). **⚠️ Surfaced: CROFT-PROTOCOL §2 domain-tagged genesis/topic pre-images diverge from `lineage-core`'s plain sha256 + `"lineage-topic-v1"` topic tag — a real reconciliation item.** | green (green-real/green-model rows; design rows deferred) |
