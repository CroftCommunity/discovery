...
fn synthetic_100k_roster_is_about_ten_megabytes() {
    ...
    assert!(
        bytes > 5_000_000,
        "roster is multi-megabyte (got {bytes} B)"
    );
    assert!(
        bytes < 20_000_000,
        "roster is ~10 MB, not absurd (got {bytes} B)"
    );
}
...
fn light_client_inclusion_proof_is_log_n() {
    ...
    assert!(
        scale::verify_inclusion(root, &roster[index], index, &proof),
        "the proof verifies"
    );

    // A wrong leaf does not verify against the same root/position.
    assert!(!scale::verify_inclusion(
        root,
        "did:key:zNotOnTheRoster",
        index,
        &proof
    ));
...
fn verification_throughput_is_positive_and_measured() {
    ...
    assert!(
        report.per_second > 0.0,
        "a positive throughput was measured"
    );
}
...
fn churn_simulation_reports_epoch_roll_and_contradiction_rate() {
    ...
    for n in [8usize, 64, 256] {
        ...
        assert_eq!(curve.n, n);
        assert!(
            curve.epoch_roll_nanos > 0,
            "epoch-roll cost recorded at N={n}"
        );
