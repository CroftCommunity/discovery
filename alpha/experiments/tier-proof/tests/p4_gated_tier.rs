...
fn grant_admits_member_from_the_grants_causal_position() {
    ...
    let req = src.put_record(&bob(), Record::Request { scope: GATED.to_string() });
    // The grant cites the request hash among its antecedents (R7 shape).
    let grant = src.put_record_with_antecedents(
        &s1(),
        vec![req.clone()],
        Record::Grant { scope: GATED.to_string(), subject: bob().did() },
    );

    let state = Fold::run(&src.all()).expect("fold");
    assert!(state.roster_members(GATED).contains(&bob().did()), "granted member is on the roster");
    ...
    assert_eq!(intervals, vec![(grant_pos, None)], "interval opens at the grant position");
}
...
fn a_non_steward_grant_does_not_admit() {
    ...
    let state = Fold::run(&src.all()).expect("fold");
    assert!(!state.roster_members(GATED).contains(&bob().did()), "a self-grant does not admit");
}
...
fn multi_steward_cosign_reaches_threshold() {
    ...
    assert!(!after_one.roster_members(GATED).contains(&bob().did()), "one of two co-signs: not yet a member");
    assert_eq!(after_one.request_status(GATED, &req), RequestStatus::Pending);
    ...
    assert!(after_two.roster_members(GATED).contains(&bob().did()), "two co-signs: admitted");
    assert_eq!(after_two.request_status(GATED, &req), RequestStatus::Granted);
}

fn msg_with(signer: &Signer, antecedents: Vec<String>, text: &str) -> Envelope {
    records::seal(signer, antecedents, &Record::Message { scope: GATED.to_string(), text: text.to_string() })
}
...
fn revocation_is_a_causal_cut() {
    ...
    let cut = src.put_record(&s1(), Record::Revocation { scope: GATED.to_string(), subject: bob().did() });

    let state = Fold::run(&src.all()).expect("fold");

    // A message whose antecedents are BEFORE the cut still verifies.
    let before = msg_with(&bob(), vec![grant.clone()], "said while a member");
    assert!(state.admit_message(&before).is_ok(), "a message before the cut is admitted");
    ...
    assert_eq!(state.admit_message(&after), Err(AdmitReject::OutsideMembership), "a message after the cut is rejected");

    // The interval set is exactly grant→cut.
    let g = state.position_of(&grant).unwrap();
    let c = state.position_of(&cut).unwrap();
    assert_eq!(state.member_intervals(GATED, &bob().did()), vec![(g, Some(c))]);
}
...
fn silence_is_not_a_verdict() {
    ...
    let first = src.put_record(&bob(), Record::Request { scope: GATED.to_string() });
    src.put_record_with_antecedents(
        &s1(),
        vec![first],
        Record::Grant { scope: GATED.to_string(), subject: bob().did() },
    );
    let second = src.put_record(&Signer::from_seed([40u8; 32]), Record::Request { scope: GATED.to_string() });

    // Fold at several later points: the unanswered request never flips to a
    // verdict. There is no timeout code path to trigger.
    for _ in 0..3 {
        src.put_record(&bob(), Record::Message { scope: GATED.to_string(), text: "time passes".into() });
        let state = Fold::run(&src.all()).expect("fold");
        assert_eq!(state.request_status(GATED, &second), RequestStatus::Pending, "unanswered request stays pending");
    }
}
...
fn leave_excludes_without_a_steward_act() {
    ...
    src.put_record(&bob(), Record::Leave { scope: GATED.to_string() });

    let state = Fold::run(&src.all()).expect("fold");
    assert!(!state.roster_members(GATED).contains(&bob().did()), "member leaves unilaterally");
    // And this is NOT a revocation — no causal cut was recorded by a steward.
    assert_eq!(state.cut_position(GATED, &bob().did()), None, "leave is not a revocation");
}
...
fn archive_rebuild_reproduces_roster_with_signatures_reverified() {
    ...
    assert_eq!(live.canonical_digest(), rebuilt.canonical_digest(), "archive rebuild reproduces the roster");
    assert_eq!(rebuilt.dropped_count(), 0, "all archived signatures re-verified");
    ...
    assert!(rebuilt_bad.dropped_count() >= 1, "a tampered archive entry is dropped, not trusted");
}
