...
| **run17-live-source** | declared-stand-in | `tier-proof/src/source.rs` (`MemSource`; `live_open_tier_leg` probe) | The open-tier fold ingests an ordered in-memory event log standing in for a live Jetstream firehose, behind the same `RecordSource` interface a live source implements. The `live_open_tier_leg` probe reports **BLOCKED** (never fabricates) when `ATP_TEST_*` creds are absent. | Genesis + self-registration written to a real PDS and ingested from real Jetstream (RUN-14's proven path). | Set `ATP_TEST_*` + egress; swap `MemSource` for the live source behind `RecordSource`. The fold is unchanged. | **Bounded — BLOCKED, reported.** Fold/reconstructability proven at component grade; the wire ingest is the RUN-14 seam. |
| **run17-multiparty** | declared-stand-in | `tier-proof/tests/p4_gated_tier.rs` (local steward/member/outsider keypairs) | The genuinely multi-party gated-tier facts (steward grant, member request, outsider) run against locally generated ed25519 keypairs, not distinct live accounts. | The two-sided facts across real, distinct atproto accounts (steward, member, outsider). | Supply `ATP_TEST_*_2/_3`; the `Request`/`Grant`/`Revocation` shapes and the co-sign threshold are unchanged. | **Bounded — intentional.** The mechanism (causal cut, intervals, co-sign) is proven; only the account provenance is local. |
| **run17-did-resolver** | declared-stand-in | `tier-proof/src/delegation.rs` (`DidKeyResolver`) | Device-key delegation resolves the delegating account as a `did:key` (no network — genuine for `did:key`) behind the same `DidResolver` interface a PLC resolver implements. | A real `did:plc` document resolved over the PLC directory (RUN-14's path). | Swap the resolver impl behind `DidResolver`; the verify + attestation + revoke-by-deletion logic is unchanged. | **Bounded — intentional.** Delegation + event-driven revocation proven; only `did:plc` resolution is stood in. |
| **run17-swarm-local** | declared-stand-in | `tier-proof/src/bin/swarm_peer.rs` (local TCP) | The swarm-peer role exchanges envelopes over a local TCP socket behind a socket seam, not the iroh overlay. | The swarm-peer runs over the iroh overlay (guardrail 6: one build attempt in-scope, else the tagged local stand-in). | A clean iroh build via the proxy upgrades the tag to a real two-peer exchange behind the same seam. | **Bounded — intentional.** Three-process co-hosting + failure isolation proven over sockets; the overlay is the upgrade. |
| **run17-rbsr** | declared-stand-in | `tier-proof/src/roles.rs` (`converge`) | The history-convergence node reconciles envelope sets by a hash-set diff over `H(envelope)`, standing in for the RUN-12 RBSR set-reconciliation construction. | Range-Based Set Reconciliation (RUN-12) for efficient large-set convergence. | Replace the hash-set diff with RBSR; the dedup-by-`H(envelope)` contract is unchanged. | **Bounded — intentional.** Dedup-across-transports proven; RBSR is the efficiency upgrade. |
| **run17-mls-loopback** | declared-stand-in | `tier-proof/steward-seal/tests/steward_group.rs` | The steward group's sealed deliberation runs over the croft-group in-proc MLS harness (real openmls ciphertext), not a real network transport. | MLS deliberation delivered over a real network transport. | The crypto is already real openmls; only the transport is in-proc (croft-group harness convention). | **Bounded — intentional.** Real ciphertext, real epoch re-key on removal; only the transport is loopback. |
| **run17-churn-model** | declared-stand-in | `tier-proof/src/scale.rs` (`simulate_churn`) | The P9(b) churn ceiling is a local epoch model: epoch-roll cost is a measured O(N) computation, the concurrent-commit contradiction rate a modelled no-arbiter collision rate. | The churn curve driven over the croft-group MLS loopback harness at scale. | Drive real `group-seal` epoch rolls at increasing N; the epoch-roll quantity is already measured O(N). | **Bounded — intentional.** The curve's SHAPE (linear roll cost, 1/N contradiction) is the deliverable; no boundary chosen. |
| **run18-retraction-local** | declared-stand-in | `tier-proof/tests/b5_retraction.rs` (harness delete event; `live_retraction_leg` probe in `src/source.rs`) | B5's middle-issue RETRACTION is the landed harness's authenticated delete event (`SourceEvent::Delete`, author-checked), not a live PDS record deletion; the `live_retraction_leg` probe reports **BLOCKED** when `ATP_TEST_*` creds are absent (never fabricates). | The content record deleted on a real PDS, its authenticated absence observed from the signed repo tree / firehose `delete` frame (upgrading B5 to live grade per RUN-18 guardrail 4). | Set `ATP_TEST_*`; run the deletion against the real PDS — the classifier (`chain::classify_existence`) is unchanged, only the deletion-evidence source swaps. | **Bounded — BLOCKED, reported.** The three-way distinction (never-existed / retracted / withheld) is proven at component grade; only the deletion's provenance is local. |

`fanout-single-run` was the second Active row; it is **retired (Reconciled, RUN-09)** — see below. Active divergences are now `hermetic-gossip`, RUN-14's two **declared stand-ins** (`run14-A2` recruiter roster, `run14-A4` service-DID `aud`), and RUN-15's seven hosting-kit stand-ins (`run15-stub-verifier`, `run15-local-root`, `run15-tf-validate` [BLOCKED, reported], `run15-bootstrap-dryrun`, `run15-sandbox-unshare`, `run15-usermode`, and `run15-s3-local` [MinIO stands in for R2 — real S3 protocol, drill PASS]), and RUN-17's seven tier-proof stand-ins (`run17-live-source` + `run17-multiparty` + `run17-did-resolver` [live atproto legs BLOCKED on creds — MemSource / local keypairs / `did:key` resolver behind the same interfaces], `run17-swarm-local` [local TCP for the iroh overlay], `run17-rbsr` [hash-set diff for RUN-12 RBSR], `run17-mls-loopback` [real openmls over in-proc transport], `run17-churn-model` [local epoch model for the MLS churn harness]), and RUN-18's one (`run18-retraction-local` [harness delete event for a live PDS record deletion — B5's live leg BLOCKED on creds]). All are sequenced by their briefs, not hidden, and none weakens a proven mechanism — each names the same guarantee's real enforcer/endpoint and the step that swaps it in.
