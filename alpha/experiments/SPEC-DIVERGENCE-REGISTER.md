...
`fanout-single-run` was the second Active row; it is **retired (Reconciled, RUN-09)** — see below. Active divergences are now `hermetic-gossip`, RUN-14's two **declared stand-ins** (`run14-A2` recruiter roster, `run14-A4` service-DID `aud`), and RUN-15's six hosting-kit stand-ins (`run15-stub-verifier`, `run15-local-root`, `run15-tf-validate` [BLOCKED, reported], `run15-bootstrap-dryrun`, `run15-sandbox-unshare`, `run15-usermode`). All are sequenced by their briefs, not hidden, and none weakens a proven mechanism — each names the same guarantee's real enforcer/endpoint and the Phase-1.5/Phase-2 step that swaps it in.
