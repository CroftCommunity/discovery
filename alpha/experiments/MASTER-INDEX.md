...
| `fed-shim/` (+ `FED-SHIM.md`, `FINDINGS-FED-SHIM.md`, `tests/specimens/`) | **The fediverse-wire conformance shim** — a slim Rust crate that produces Mastodon-shape bytes (Follow / Undo Follow / Delete(Actor) activity JSON, actor-document JSON-LD, draft-cavage HTTP signatures) with byte-exact fidelity to captured specimens. Wire-conformance surface, NOT a Mastodon replica: firm non-goals include persistence, retry queues, Sidekiq, HTTP transport, WebFinger, media, Mastodon REST API, streaming, moderation, actual Ruby/PG/Redis behavior — closed by the attended live leg with GoToSocial / real Mastodon. Path-deps `ap-ambassador::verify` (dev-dep only) for a cross-implementation roundtrip test — two independent implementations of the draft-cavage HTTP-signature scheme agreeing on the same wire bytes. Retroactively upgrades RUN-AP-01's P1 fixture leg to a specimen-anchored **shim leg** (`ap-ambassador/tests/shim_leg.rs`, 3 tests). | **RUN-FED-SHIM-01** (11/11 green red-first, `Modeled`; five wire specimens dated + citing Mastodon source paths; F-AP-2 surfaced by the shim leg — AP JSON parser array-skip fix — filed against `ap-ambassador::verify::parse_json_object`) |
| `ap-ambassador/` (+ `AP-AMBASSADOR.md`, `FINDINGS-AP.md`) | **The ActivityPub-ambassador receipt lane** (own-lane RUN-AP-01) — a Croft-product-lane aligned offering that ingests inbound AP activities (Follow / Undo / Delete), verifies them at the HTTP-signature layer (Mastodon-shaped: draft-cavage-http-signatures, RSA-SHA256, digest as `SHA-256=<b64>` of body), mints an evidence-complete receipt over the SAME §4.6 dag-cbor envelope path attest-family uses, and derives a follower-interval roster by folding Follow/Undo pairs. Delivery-plane role in the A.7 sense: no ordering authority, no membership authority, and — pinned this run — no governance conductivity (P5 permanent-red pair, structural + behavioral). Undo Follow = second receipt; Delete = redact body + keep skeleton (masked never-was-world equality; state → `attested-redacted`; re-verify returns distinct `EvidenceRedacted`). Identity upgrade to atproto = subject-initiated dual-proof binding only (fresh-start default; no auto-link; grade derives from binding, not from gateway observation). Reuse: attest-family path-dep, canonical dag-cbor unchanged, closed `AntecedentKind` enum the P5 compile-boundary; NO register-file edit, NO drystone-spec edit. | **RUN-AP-01** (P1–P7 green red-first, 37 tests, `Modeled`; no live fediverse leg — fixture RSA keys, in-test resolver, in-memory store; findings F-AP-1 receipt-`state`-not-in-id-encoding; AP-OC-6..OC-9 surfaced, not decided) |
...
| `lexicon-community/` (+ `AMBIGUITIES.md`, `SPEC-BADGE-BLUE.md`, `../attest-family/ENGAGE-LEX.md`) | **Lexicon Community engagement lane (own-lane RUN-LEX-01)** — a clean-room, second-implementation spike against the published CID-First Attestation Specification (badge.blue, Gerakines) + the lexicon.community calendar/RSVP/location lexicons. EXP-LEX-01: fixture corpus + schema harness, golden-validate/adversarial-fail red-first, gated by the official `@atproto/lexicon` tooling (validator of record) — surfaced A-6 (official tooling IGNORES unknown fields → a smuggled scalar validates). EXP-LEX-02: lossless consumption of REAL PDS records (recomputed CID == authoritative CID; byte-identical DAG-CBOR; cross-repo strongRef drift detected). EXP-LEX-03: the clean-room verifier (inline + remote, repo binding, did:key over 3 curves, low-S) built from spec text alone and proven INTEROPERABLE with the reference `atproto-attestation` crate's own signed output — the interop probe empirically pinned the spec's underspecified signing input (36-byte binary CID, SHA-256 ECDSA), and demonstrated A-1 (inline attestations authenticate a key, not an issuer, until a DID-doc binding check is added — shipped as the strict posture). EXP-LEX-04: the differentiator — holder-stapled status (CT/RFC-9162 design ported from RUN-ATTEST-04 onto the spec's remote shape, on SHA-256 so it composes), freshness + revocation with ZERO verifier callback; candidate `community.lexicon.attest.{treeHead,holderBinding,inclusionStaple}` DRAFTs; 870 B staple benchmarked vs a callback. EXP-LEX-05: Lenses (Polite Goshawk) seam downgraded to a worked example (WG has no concrete format, repo 404). Everything rides single-author records (the thread's scope line); `ing.croft.*` stays the Drystone home, candidates proposed for `community.lexicon.*` via the WG process. | **RUN-LEX-01** (EXP-LEX-01..04 green red-first — 33 tests + official gate + one-command demo, `Modeled`/interop-verified; RED sha256 `08d7dc56…`, GREEN `e7598662…`; ten-row grounded AMBIGUITIES feedback table incl. two security-relevant (A-1 issuer↔key binding, A-8 remote-has-no-signature); EL OC-1..5 surfaced, not decided — engagement/posting is the owner's call) |
| `hist-atproto-spike/` (+ `HIST-ATPROTO-MATCHUP.md`) | **HIST lane (own-lane RUN-HIST-01)** — a PDS repo as the history store's durable dataset backend, thin fetch-and-caching layer in front. Part A: eleven-row required-abilities brief, every current-ATProto claim anchored to a primary source fetched in-session 2026-07-20 (repository/record-key/blob/sync/lexicon/event-stream/data-model/xrpc specs, the live listRecords lexicon JSON, PLC spec, migration guide) — carries the row-1 publicity fact (PDS-resident blobs REQUIRE a public referencing record), the listRecords no-range-bounds gap, the delete-residue bound (metadata-shaped, not content-shaped), the ordering rider, draft `ing.croft.hist.{entry,checkpoint}` lexicons, and the five-entry HS OC register. Part B: the mechanical-seam spike, B1–B7 red-first (envelope↔record lossless, rkey order incl. padding boundary, gap-over-cursored-pages by bounding digests, fold-order rejection with a compile-level cursor boundary, CAR re-hydration with named incompleteness, omission detection reusing the A2.3 `backfill_import` unchanged, checkpoint prune gate with §L's construction left open); live blob-lifecycle legs attended-confirmed against the real bsky.social PDS | **RUN-HIST-01** (Parts A+B green red-first, 23 tests + 2 compile_fail doc-tests, `Modeled`; live legs observed (blob temp storage, re-upload no-op, mandatory reference, cleanup); finding F-HIST-1 (per-group-DID enumerability); **all five HS OC tags pending** — no owner call decided). **RUN-HIST-02 rev B** — the gentle LIVE PDS legs at `spike/hist_live/` (own dir): all **E1–E8 green** on a throwaway bsky.social account (budget-capped 100-record/3-blob, 1 rps, full teardown + CAR archive), **E1/OC-2 GREEN** (a repo record's CID serves directly as the reconciliation byte-head, **zero re-hash** under atproto canonical dag-cbor), `rkeyStart/rkeyEnd` found deprecated/non-filtering (`sync.getBlocks` is the range primitive); merged to main (PR #30) |
| `wasm-seal/` (7 crates + Node hosts; `RUN-19-SUMMARY.md`) | **Sealed tier in the browser's shape (own-lane RUN-19)** — MLS in WASM exchanging messages over real QUIC (WebTransport): OpenMLS RustCrypto compiled to `wasm32-unknown-unknown`; cross-build interop goldens (wasm-sealed ciphertext unsealed natively and vice-versa, byte-identical group state); encrypted state-at-rest survives a wasm host restart; real QUIC through a **content-blind DS** with offer-gating (the browser reaches the blind DS through the front door — no iroh bridge needed); two wasm members exchange over the blinded path. P6 amends the landed §A.8 "deferred, needs a relay bridge" sentence via staging (WebTransport Baseline since 03/2026; the bridge was only for peer reachability, which the browser doesn't need). Node-hosted stand-ins named (headless-Chrome attempt then tag; native QUIC client speaking the identical WebTransport wire). | **RUN-19** (P1–P7 green red-first, `Modeled`; digest-attested — no toolchain in the audit container; §A.8 upgrade staged in `proposed-changes-*`; MASTER-INDEX row added 2026-07-20 by the capture audit) |
...
| `croft-relay/` — **moved to `croft-stack/relay/source/` 2026-08-10** (crate renamed `croft-relay-admit`; design superseded-in-part by `../plans/2026-08-07-1-plan-croft-relay-tiered-admission.md` — budget-and-drop, not tier buckets) (+ `DESIGN.md`, `docs/adr/0001-0004`, `OPEN-QUESTIONS.md`, `RUN-CROFT-RELAY-01-SUMMARY.md`) | **atproto-gated iroh-relay admission (own-lane RUN-CROFT-RELAY-01)** — the app-side enforcement core for calling: registered-only reception, a coordination tier (holepunch survives, sustained relayed media starved), and a broker tier, as three dials on one credential. Built crate `croft-admit` is relay-agnostic (no iroh dep): DID-control enrollment against `ing.croft.iroh.endpoint` (mocked PDS, deny-closed), an axum `/access` service answering iroh-relay's HTTP-hook contract, JWT/EdDSA per-endpoint capability tokens with a three-gate verify (signature / expiry / `sub`==authenticated-endpoint anti-replay) and full deny matrix, and a `tier`→`RateBucket` mapping. Phase-0 embed-vs-fork decided against **real `iroh-relay 1.0.0-rc.1` source** (public `AccessControl` trait ⇒ embed feasible; per-connection rate override the one upstream gap; header correction `X-Iroh-NodeId`≠doc'd `X-Iroh-Endpoint-Id`). | **RUN-CROFT-RELAY-01** (Phases 1-3 app-side green red-first, 36 tests, `cargo mutants` 0 survivors / 52; `Modeled`; 3 declared stand-ins incl. `SPEC-DELTA(phase-3-calibration)`; five §7 owner-calls open). **RUN-CROFT-RELAY-02** — GitHub git access opened mid-project: cloned iroh, built + ran `iroh-relay 1.0.3 --features server` (**80/0** baseline), and built `croft-relay-embed` — the `AccessControl` adapter over real iroh types (7 tests, anti-replay hinge exercised over an authenticated `ClientRequest`; workspace 43 total). ADR-0005. ADR-0001 findings re-verified on 1.0.3 (header `X-Iroh-NodeId` + no-rate-on-`Allow` gap both persist). ONLY the live two-endpoint holepunch + bucket calibration remain deferred (needs a running relay). **RUN-CROFT-RELAY-03** — the local live leg: `croft-relay-embed/tests/live_relay.rs` spawns a real `iroh-relay` Server on localhost gated by our `TokenAccess`, drives real relay Clients through it (valid admits + pings; bogus + cross-endpoint-replayed tokens denied at the handshake), and forwards a datagram **A->B through our gated relay** (payload+origin intact); a relayed contact round-trip is byte-accounted as a calibration datapoint (localhost, not the holepunch-disco total). Workspace 46 tests. ADR-0004 calibration section updated; `SPEC-DELTA` stays. Only the real NAT'd holepunch-disco calibration remains. |
