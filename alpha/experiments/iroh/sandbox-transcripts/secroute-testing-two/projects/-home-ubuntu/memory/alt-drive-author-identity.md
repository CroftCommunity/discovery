...
description: "For alt.drive commits the user's git identity is \"Chase Pettet <chase@owasp.org>\" — not chase.mp@owasp.org (stale ref) and not the work [work-email] identity."
metadata: 
  node_type: memory
  type: reference
...
Why the warning: the box's default git identity may be `[work-email]` (work work email); without the override commits land under the wrong identity.
