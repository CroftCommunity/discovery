...
| **Live two-endpoint leg (holepunch)** | **Deferred — needs a running relay** | Stand up the relay, prove endpoint A reaches B through it, holepunch. The ONLY remaining piece of Phase-0/Phase-3 acceptance; needs live networking + NAT traversal, beyond clone-and-test. |
| **Phase-3 bucket calibration** | **`SPEC-DELTA(phase-3-calibration)`** | Instrument a real holepunch coordination exchange, measure its bytes, re-derive the coordination bucket (headroom above measured, far below media bitrate). Update `tier.rs` + the pinned regression test together. Needs the live relay. Mapping + application point already built/tested. |
...
| **Sibling `xmtp-ambassador`** | **Future — RUN-FS-01 open-questions doc drafted (owner walk needed)** | Owner-walk pre-brief at `RUN-FS-01-OPEN-QUESTIONS.md` enumerates FS-V1..V5 with proposed answers and tradeoffs; execution follows the walk. XMTP dev-dependency (`libxmtp`) needs a compile-fits-env check before P1's tests are authored. The same delivery-role charter (§0 governing principle) applies verbatim; the abstraction is what the run validates. |
| **`fed-shim` wire-conformance shim (RUN-FED-SHIM-01)** | ✅ **Done (2026-07-20, red-first, 11 tests, `Modeled`)** | Slim Rust crate producing Mastodon-shape bytes with byte-exact fidelity to captured specimens (Follow / Undo Follow / Delete(Actor) / actor doc JSON-LD / HTTP-Signature header). RUN-AP-01's P1 fixture leg upgraded to a specimen-anchored **shim leg** (3 shim_leg tests). F-AP-2 surfaced and fixed — parse_json_object now skips balanced arrays (Delete's `to` field). Firm non-goals per FED-SHIM.md §3: persistence, retry, HTTP transport, WebFinger, media, Mastodon REST API, streaming, moderation, PG/Ruby/Redis semantics — closed by the attended live leg (GoToSocial or real Mastodon on a real host). |
| **Attended live leg — shim ↔ real Mastodon or GoToSocial (BLOCKED — attended run)** | **BLOCKED (attended host + disposable AP instance)** | The gated live leg per FED-SHIM.md §4. Two shapes: (a) shim ↔ Mastodon docker-compose on a real host; (b) shim ↔ GoToSocial single-binary. Predictions: outbound shim Follow → 202; inbound real-Mastodon Follow → shim's `accept_inbox_post` returns `Ok(VerifiedActivity)`. Byte-level: specimens under `tests/specimens/` match observed real-Mastodon output; any diff records as a specimen update + shim update + naming the change in the run summary. |
