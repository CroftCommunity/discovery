...
def blob_prefix(m, blobdir):  # D3: no per-service prefix yet (D6 adds it)
    return blobdir.rstrip("/")


def state_path(m, dbfile):    # D3: no per-service prefix yet (D6 adds it)
    return dbfile
...
def render_litestream(manifests, out):
    lines = [
        ...
        "# NOTE: replica 'path' has no per-service prefix yet — D6 adds it.",
        "dbs:",
    ]
    for m in manifests:
        data_dir = f"/var/lib/{m['name']}"
