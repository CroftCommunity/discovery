...
| forage.fyi | `CroftCommunity/forage` | **Behavior-scale mock** of the community-discussion forum pad (Reddit-shape): persona-switch PWA, no backend. Named **Forage** 2026-08-21 (NAMING.md forum-layer; was Graze → Amble); renamed + cut over from `graze.ing` 2026-08-23/24 (graze.ing retired dark, E73); rook-and-wreath brand + warmed palettes live | the forum build (method `thinking/behavior-scale/`; spec `thinking/app/build-specs/graze-persona-switch-spec.md` — spec filename unchanged, historical; large-tier fork in COHESION §63) |
...
| Graze Social | **Graze** (graze.social) | Custom-feed builder for Bluesky ("own your algorithm"): block-based feed construction + remixing, See-More/See-Less adaptive per-user filtering (25 See-Less clicks on an author → global per-user filter, per the dialogue), anonymous aggregate operator metrics; US public benefit corporation, outside investment, **sponsored content on custom feeds** as the revenue model | live `[UNVERIFIED details]` | contextual / learn↔ — **the name collision that forced the forum project's Graze→Forage rename** (owner: "led to graze rename"; COHESION §79); also a live experiment in feed-operator monetization worth watching against the zero-take stance |
| Inventaire association | **Inventaire** | Linked-open-data book catalog on Wikidata + open bibliographic sources: personal physical-library tracking, lend/trade listings, reading lists under open-data principles | live `[UNVERIFIED]` | learn↔ — joins §5k's book-review ladder on the open-by-license rung (structured LOD rather than social reviews) |
