...
## 67. 2026-08-07 — cost ceilings + the prepaid meter as equity instrument; and the CISS file-sync client lane opens

**Incoming:** a claude.ai design dialogue (`seeds/transcripts/raw/ciss-cost-ceilings-and-prepaid-meter-equity-2026-08-07.md`, cleaned-paste — §4) plus a live design pass on building a **file-sync client** over CISS. Distilled → `thinking/cost-ceilings-and-the-prepaid-meter.md`; principles → `crystallized/principles.md` (two new Tier-3 bullets); backlog → ROADMAP_TODO **E89** (ceiling/prepaid design) + **E90** (sync client). The metering half **extends §65** (the proven D5 charging mechanism) with the dimension it left implicit: not just *how you charge* but *how you cap*.

**CLOSED — the ceiling is mechanical, and it is an equity argument, not a convenience.** A *synchronous* meter (receipt-per-byte-crossing at the boundary) makes a hard spending cap a **ledger comparison before serving** — impossible under incumbent batch billing. Expressed in the **dial pattern**: co-signed "spend stops at X," throttle/defer past X, **never mint debt**. Rent can't spike (`manifest × tariff`), so the cap governs postage; the **client cost twin** enforces the same ceiling pre-flight → two-sided. The **inversion** (who carries risk / variance-vs-predictability / discretionary-vs-on-book mercy) supplies the *why* under the existing **grace ledger** (`cooperative-social-union-model.md`). The prepaid meter's dual history (UK scandal `[UNVERIFIED]`) yields a 5-rule design checklist; the standout structural rule = **exit is unconditionally cap-exempt** ("they can never keep your furniture") → a **candidate new CISS invariant**.

**OPEN — bug vs design-gap (per `CISS/CLAUDE.md`).** Most rules are **design intent not yet an invariant**: (1) exit-exempt self-egress → write into `CISS/docs/SECURITY-POSTURE.md` (small ADR); (2) co-signed ceiling/mode → depends on the **bilateral-receipt seam** (today `501`, E82 lane) + an I5-style monotonic dial record; (3) tariff parity + throttle-count-as-governance-metric → co-op **bylaw/governance**, not server code; (4) client cost twin → buildable now (`du` × published tariff). External facts (McCarthy/Waldrop wording, EU Data Act dates, UK Ofgem/British Gas figures) are **model-asserted `[UNVERIFIED]`** — a cloud-metering `research/` note could ground them (E89).

**NEW LANE — CISS file-sync client (E90).** A cloud-storage sync client treating files as CISS items. Grounded against the **real CISS API** (v0.5.6, mapped this session): 2 MiB per-object cap ⇒ **chunking mandatory** (FastCDC, chunk-level resume suffices); `du`/`listBlobs` = a **have/want set for free** (no new endpoint); the signed keep-set **Manifest already has monotonic-seq CAS (I5)** ⇒ safe multi-device commit with **no server change**; content-addr + read-reverify = the integrity anchor. **Client invents one artifact**: a filesystem manifest (`path → chunk cids`) serialized as its own content-addressed blob; CISS `Manifest` keep-set = ∪ chunk cids + fs-root cid. **Both transports as options** via a `BlobTransport` trait + a **sha256(CISS)/blake3(iroh) dual-name** bridge in the manifest — iroh's real prize is **peer-fetch → less server egress → less metered cost** (ties §67-metering). **DECISIONS (user 2026-08-07):** CISS stays **indifferent** (dumb content-addressed byte store) → **encryption is a client-only layer, no server impact** (fork collapsed); **timestamps are an assertion, never authoritative between nodes**. Scope = the **own-device** file surface (one device + helper → optional-helper second device); cross-lineage groups out of scope. **Design SETTLED + plan written** (`CISS/docs/plans/2026-08-07-file-sync-client.md`) after a 4-agent corpus research pass: (a) rename detection = client reconciler, **exact chunk-set only** (partial not useful); (b) **HEAD = per-device signed `Frontier` folded locally**, never asserted/server-minted — the corpus deliberately rejects accepting an asserted HEAD even among own devices (self-verifying frontier exchange + fold-or-fork; the meer is a content-blind helper; set-reconciliation is the shared primitive across chat/governance/files but NOT canonical merge — `thesis-lineage-groups.md:25`; CISS is already spec'd as one-store-two-consumers, `plans/2026-07-31-1-…:43`). Model: `DeviceHead` blobs + `Frontier{heads}` in the seq-CAS'd (I5) manifest slot; **non-lossy** commit (each device writes only its own `heads` slot); current tree = a client-side **deterministic fold** (per-path 3-way merge, conflict-copy, content-address tiebreak). **Identity:** shared account key now → MLS/lineage per-device keys later (option). **Only server change in the ladder = one additive owner-signed `Manifest.heads` field (M3)**; server stays blind. Milestones M1→M5 (backup/restore · bounded footprint · 2-device converge · iroh peer-fetch · cost twin). **First concrete instance of the corpus's parked frontier machinery** (Drystone `LocateLatest`/completeness-ahead), at the tractable dataplane dial. Ties **E82**/**D5**/**E85**, `multi-device.md` (frontier exchange + same/cross-lineage tiers), `meer-superpeer-design.md`, `realtime-media-over-iroh.md`, `beta/drystone-spec/`. **Status 2026-08-07 (later session): M1 SHIPPED** — `ciss-sync` crate + `ciss-ctl sync backup|restore` merged to CISS main (PR #14), byte-identical backup/restore proven end-to-end (have/want dedup, keep-set under I5, verified restore + cold discovery, tamper fail-closed); execution plan + a Drystone/Croft foundations review (deviations owned: sha-256-native with the blake3 bridge, `kind` self-tag as §4.2-spirit domain separation, content-*indifferent* ≠ corpus content-*blind*) in `CISS/docs/plans/2026-08-07-file-sync-m1-chunk-and-backup.md`. E90's "bsky app-token" auth line reconciled there: the write path structurally needs the namespace key (`id:` session); the app-token path is read-side.

## 68. 2026-08-07 — the meer stops needing a store: Phase-10's convergence component unifies with the CISS lane

**Incoming:** a live Claude Code design session (`seeds/transcripts/raw/meer-store-and-forward-custodian-queue-2026-08-07.md` — user turns verbatim, assistant turns content-faithful, §4). Distilled → `thinking/meer-as-custodian-queue.md` (hypothesis), `plans/2026-08-07-meer-lane.md` (lane), `experiments/meer-queue/SPIKE-SPEC.md` (Phase 0); backlog → ROADMAP_TODO **E91**. **Supersedes the storage half of `thinking/meer-superpeer-design.md`** (2026-06-16); its roles, confidentiality tiers, and anti-entrenchment analysis stand.

**CLOSED — "find an MLS delivery service to adapt" was the wrong search, and the absence is structural.** RFC 9750 deliberately leaves the DS unspecified, so there is no spec to implement and every shipped DS is bespoke to its product — in practice a DS *is* the server half of a messenger (identity, group creation, KeyPackage hosting, Welcome distribution, room policy, ordering). OpenMLS ships a labelled proof-of-concept; `phnx-im/air` is a real one inside an AGPL-3.0 application (with a DS/QS split that independently matches our queue/store seam); MIMI standardised the role by reintroducing a per-room **ordering hub** — precisely what Drystone declines (`beta/cairn/mls-and-mimi.md`). Strip ordering and group-state validation and a **mailbox** is what remains, which is why the shim is genuinely small.

**CLOSED — Phase 10's gate binds the history store, not the meer.** `plans/croft-stack/10-drystone-layer.md` gates the convergence server on "the fold/MLS becoming real." Because the per-author index is **sealed** (Part 2 §6.6.2), the meer cannot order and does not need to, so it carries **zero fold knowledge** and is unblocked today. The history store still needs G-hist, nested double-sealing, and an envelope whose byte layout is `[gates-release]` — genuinely gated, genuinely a different component.

**SEAM CLOSED — E82's "one store, two consumers" becomes concrete, and the second consumer is not what §65/§67 assumed.** CISS gains a **typed-chain substrate** (chain kinds, custodial write, owner-declared ceilings, per-chain metering) and the meer is a **thin gateway service** over it. The **chain-kind** mechanism generalises §67's `kind` self-tag from a *blob-format* tag (client-side, in `ciss-sync`) to a *chain* property declared in the **owner's manifest slot** — never in a head, since a head-borne kind would be self-authorizing for the custodian — fixed at genesis, bound into the signed preimage the way `heads` already is, with **`queue` the only custodially-writable kind and unknown kinds failing closed**. Custodial writes live in a **separate custodian-signed record**, so B1/B3 keep their proofs and revocation is an ordinary owner write inheriting B3 anti-rollback. This is the **write-side mirror of the gated-reads `PolicyRecord`** — whose own Problem Statement already named the history-convergence backend as its motivating consumer.

**NEW — custody is a dial, sibling to the confidentiality dial.** `meer-superpeer-design.md` established *what the meer can see* (Tier 0/1/2/no-mirror). This adds *who owns what it holds*: **per-DID queues (default)** versus **meer-owned pooled**. Per-DID **retires rather than satisfies** the state-portability anti-entrenchment guard — there is nothing to port because the mail never left home; portability is a promise a helper gives your data back, ownership is not having handed it over. Pooled wins for bootstrap (no account yet), idle-heavy scale, and anyone who would rather the meer not know them by a durable identifier. The trade is symmetrical and cannot be maxed on both sides.

**NEW — the transit meter closes an open edge as a byproduct.** `meer-superpeer-design.md` names the **offline-data fraction** as the only number that sizes a meer fleet and lists it under open edges as *unmeasured*. Custodial per-DID metering **is** that measurement. Corollary flagged in session and still **OPEN**: the mail purges at 14 days but **the meter does not unless we say so**, which would leave the meer's most sensitive artifact as the only thing that never expires (ties the metadata-transparency guard, item 6 of the same doc). **Meter ≠ bill** — keeping transit unbilled also defuses the "spam costs the victim money" consequence of mail landing in the recipient's namespace, leaving the owner-declared ceiling to defend.

**Resolved against §67's grain:** drain authorizes on **CISS account identity, never MLS identity** (presenting group credentials to a blind store would leak group membership; entitlement is enforced by the seal, not the drain gate). And **no cursor** — the have/want diff over content-addressed blobs *is* the position, which is Part 2 §6.8.1 ("the cursor and the gap-detector are the same object") arriving from the storage side; **deliver-once is correct because §6.6.5's device-Group is the fan-out**, with race-mode when no device group is present.

**OPEN — Phase 0 is a discovery phase, specified but not run** (`experiments/meer-queue/SPIKE-SPEC.md`). Runs against **plain CISS as it exists** with three registered stand-ins, and **extends `iroh/crates/mls-welcome-over-iroh`** (real OpenMLS 0.8.1), so no new MLS client is needed. **S8 is the scenario that may reshape the design:** MLS object sizes against `MAX_OBJECT_BYTES` = 2 MiB — a cap that is **not arbitrary** (it came from the 2026-08-03 security review's memory-exhaustion finding, so raising it re-opens a closed vulnerability unless streaming replaces buffering, and it is the same cap that forced §67's FastCDC chunking). `GroupInfo` with the embedded ratchet tree is **O(N)**, so `Welcome` is the object at risk — and **Part 2 §6.9.1 already mandates shipping the tree out of band for the broadcast tier**, so the storage cap may simply be re-discovering a constraint the spec resolved from the protocol side. Also open: who mints the custodian grant (per-persona lean per §5.4, vs Group-collective); meter retention; custody-dial default per Group.
...
## 74. 2026-08-21 — croft-stack's admission/usage journal and the spec's meer are two growth paths to one helper
