...
- **The utility-representative graph vs the happenstance graph (and how it surfaces without fatigue).** A verified "someone asserting that they know you," paired with a brief scoped descriptor ("friend from school"), is a way to grow a graph representative of *actual utility* rather than happenstance or broad strokes — the antidote to the platform pattern where "same high school + same volleyball league" is asserted as connection when there is no real social overlap (which is fine — np). The design risk is not the assertion but its surfacing: if such overlap constantly raises a notice, or has *any* meaning auto-inferred, it is exhausting. Prefer a **permanent, low-noise overlay** that informs only when a connection is genuinely meaningful, with the person controlling how open they are both about themselves and about the others connected to them. Note the mechanics are no longer hypothetical: the "knows you" / vouch / scoped-descriptor / per-persona-visibility model is a **built primitive family** — the attest lane (`../experiments/attest-family/PRIMITIVES-ATTEST.md`: attestation as the mutual co-signed edge = "the friend case", `vouch`, `scope` = the descriptor, `resolvability` = per-persona visibility control, `anchor persona` = the hard-split multi-persona case for work/home / school/home). So this question is now "how should the built primitives *surface* as a low-noise utility graph," not "can we build them." Couples S3, S4, and §3's label layer.
