...
pub const GET_TIMELINE_LEXICON: &str =
    include_str!("../lexicons/org.croftc.experiment.feed.getTimeline.json");
pub const GET_POST_THREAD_LEXICON: &str =
    include_str!("../lexicons/org.croftc.experiment.feed.getPostThread.json");
...
impl Lexicon {
    ...
    /// Validate a query response against this lexicon's `main.output.schema`.
    /// Used to assert that a served AppView view matches its read lexicon.
    pub fn validate_output(&self, value: &Value) -> Result<(), String> {
        let main = &self.doc["defs"]["main"];
        if main["type"] != serde_json::json!("query") {
            return Err("lexicon main is not a query".into());
        }
        let schema = &main["output"]["schema"];
        self.validate_field(schema, value, "output")
    }
        ...
        match schema["type"].as_str() {
            ...
            Some("integer") => {
                ...
                if let Some(min) = schema["minimum"].as_i64() {
                    if n < min {
                        return Err(format!("`{name}` below minimum {min}"));
                    }
                }
                if let Some(max) = schema["maximum"].as_i64() {
                    if n > max {
                        return Err(format!("`{name}` above maximum {max}"));
                    }
                }
                Ok(())
            }
