...
///
/// A BLAKE3 hex digest stands in for a real CIDv1 (which would be a multihash
/// over the DAG-CBOR encoding). Sufficient to prove the strongRef wiring; the
/// shape (`cid` string alongside `uri`) matches atproto.
pub fn cid_for(record_json: &str) -> String {
    format!("b3-{}", blake3::hash(record_json.as_bytes()).to_hex())
}
