...
- If not, build a reference fold **faithful to R1 through R4** and label it a reference fold, not production. A green run now means "an R1-through-R4-faithful fold is order-independent and causally correct," which is real evidence about the specified semantics, but it is still not evidence about a production implementation, because none exists in this repository (`croftc/upstream-repo`).
