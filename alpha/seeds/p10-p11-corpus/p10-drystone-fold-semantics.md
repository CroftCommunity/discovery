...
`Scope: specifies how the governance fold resolves conflicts, so that the fold is a deterministic, order-independent function of the set of authorized governance facts. Answers OQ-1 (causal order versus hash tiebreak), OQ-2 (role cascade on member removal), OQ-3 (operations on absent targets), OQ-4 (repeated threshold changes). Feeds the load-bearing convergence property of p10-drystone-scaling-and-ordering.md.`

`Companion to: p10-drystone-part2-mechanics.md (§7.3.1, the merge target), p10-drystone-governance-finality.md (whose A1 amends R1 below, folding assembled quorums rather than individual votes; the two should merge together into §7.3.1), p10-drystone-fact-and-chain-representation.md (which specifies the fact the fold consumes and the observed-head reference R1 walks; concurrency here is derived from the local DAG index defined there), p10-drystone-scaling-and-ordering.md (whose governance-ordering claim rests on the order-independence established here), p10-drystone-asset-keying.md, p10-drystone-history-durability.md.`
...
Amendment (see p10-drystone-governance-finality.md A12): in the layered fold, a removal's cascade (the ceiling stamp and the role revocation above) rides at the removal's tier (membership removals, tier 2), not deferred to a later tier. So by the time role grants (tier 4) resolve, a removed member already has no roles to reason about, and the effective-roles projection is consistent at every tier boundary, not only at the end. `Design.`
