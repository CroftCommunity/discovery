...
`Companion to: p9-drystone-asset-keying.md (the per-scope content keys and store flavors this doc's records are sealed and classified by) and p9-drystone-history-durability.md (the MLS transport, per-author chains, and content-blind store that carry private records here). Also companion to p9-drystone-part1-principles.md and p9-drystone-part2-mechanics.md; vocabulary inherits from the conventions-and-decisions reference.`

This is the "what" for the social-application surface: the mapping between Drystone's private, membership-scoped model and the public ATProto ecosystem, and the constructs that let the two present as one product without sharing a store. Normative keywords are **MUST / SHOULD / MAY** (BCP 14). Status flags per load-bearing claim: `Verified` (checked against a primary source; where the check was performed in the external research pass against the ATProto specifications rather than directly this cycle, it is marked `Verified, research pass`), `green-real` (exercised against a running system), `design` / `Synthesis` (the design's own reasoning), `[confirm]` (load-bearing and resting on an unresolved external or internal item).
...
The research classified the public lexicon, and it maps onto the asset-keying §E flavors as follows. `Verified, research pass` for the per-record classification.
...
**What threadgate and postgate are.** `Verified, research pass`: threadgate lives at the root post's key and restricts who may reply, via an allow list of rules (mentioned actors, your followers, accounts you follow, members of a named list), where an empty list permits nobody and an absent record permits anyone; it also carries a hidden-replies list. postgate governs who may quote or embed a post. Both are entirely public: they gate who may write public content, and a reply that passes the gate is still a world-readable public post. They are not confidentiality mechanisms.
...
|---|---|---|---|
| Repo equivalence (§B) | ATProto repo is an append-only log | Per-author keyed store, unioned; substrate is LWW, immutability is app discipline | atproto.com/specs (Verified, research pass) |
...
| Lexicon (§D) | Find an existing group lexicon to copy | Reuse the content lexicon, supply the group layer custom | no Bluesky group primitive (Verified, research pass) |
...
| Professional axis (§J) | Different platforms need different lexicons | One lexicon, parameterized by policy and defaults | Verified, research pass (D-5) |
...
- **The private-AppView topology is only as safe as its discipline.** It is safe when overlays stay sealed and the AppView serves ciphertext; it becomes a single point of confidentiality failure the moment it serves cleartext to authorized readers. The confidentiality of the AppView-provisioned-audience case rests on an unresolved item (Open items).
...
- **Membership view versus Arbiter projection reconciliation (§I).** How the private governance-chain member list and a public Arbiter-style XRPC projection are kept consistent for interoperability without the projection becoming a second source of truth. `design/Synthesis; open.`
...
- **MST fanout (minor).** The research flagged a documentation conflict: the ATProto specification states a Merkle-tree fanout of 4, while a widely-circulated implementation package doc states 16; the specification governs. Noted only so public-interop implementers trust the spec. `Verified, research pass.`
...
- **Provenance.** The ATProto and platform facts here are `Verified, research pass`: verified in an external research pass against atproto.com/specs and the platforms' own documentation, not re-fetched this cycle. The MLS and Willow facts these constructs rely on are `Verified` directly, in the companion docs (asset-keying §M-equivalent and history-durability §M). The mapping onto Drystone flavors and governance, and the overlay and annotation constructs, are `design/Synthesis`.

- **Companion linkage.** Records here are sealed under the per-scope asset keys of p9-drystone-asset-keying.md and carried by the transport and stores of p9-drystone-history-durability.md. The private-overlay distribution topologies (§H) are a specialization of the history-durability tier model. Repoint against Part 2 §5.7 (k-of-n membership) and §7.6 (fork) when this material is folded.

- **Tag posture.** Nothing here is `green-real`. External facts are `Verified` or `Verified, research pass`; constructs and mappings are `design/Synthesis`; the AppView scope key, chrome CRDT scoping, membership-projection reconciliation, and moderated-post admission are open or `[confirm]`.
