...
| **A privacy-preserving usage-measurement kit (Claude, 2026-07-23)** — **edge counter over a declared graph** (counters not logs; `nav.a__to__b`); registry-generates client+panel+fixtures; disclosure panel double-duty (rich local view vs exact outbound counts); `expires` honored at runtime; Poplar/VDAF heavy-hitters noted-and-skipped (needs 2 non-colluding operators); CNIL 2025 three-event ceiling | `raw/usage-measurement-edge-counter-privacy-kit-2026-07-23.md` | **preserved-condensed (cleaned-paste, §4) for the dialogue; RUN-MEASURE-01 (E0–E8) preserved VERBATIM** (pasted in full). **A NEW cross-property helper concept** (arecipe = recommended pilot); quantitative claims (sendBeacon 95.8%/82.9%, 64 KiB cap, CNIL 2025, Litestream ~2.6M PUT/mo) `[UNVERIFIED]`, Phase-2. Ties to Skylite's sponsor-vs-child retention stance + the existing Caddy/SQLite/Litestream/R2 kit. COHESION §58. |

**Carried-forward + open decisions (surfaced, not resolved — the user's calls):** the un-pasted RUN files
(`timers.zip`, `wiki.zip` — six instruction docs) are **referenced-but-not-captured**; capture verbatim under
`seeds/*-unpacked/` if the archives are provided (matches `arecipe-unpacked/`). The Bluesky origin dates +
"~40M scale" and the arecipe/measurement technical claims are **`[UNVERIFIED]`, Phase-2 fact-check**. The
measurement kit's **name and home** are unresolved (placeholder), as is its **default local-history
retention** (the Skylite sponsor-vs-child call). Connective tissue this intake: `COHESION.md` §54–§58;
`ROADMAP_TODO.md` §E; `beta/OPEN-THREADS.md` T61; `ECOSYSTEM.md` (Bluesky provenance note, Wikibooks/MediaWiki,
Umami/Poplar/CNIL); `beta/socialization/kindred-work.md` (Jay Graber); and the two beta socialization
distillations. Not committed.
