...
| **croft-relay — tiered admission, fork-vs-embed, member-sponsored tunneling (Claude Code, 2026-08-07)** — the design session that followed RUN-CROFT-RELAY-03/PR #40. Walks the owner's opening "fork iroh-relay on a maintained branch" instinct to a **no-fork** design: split `RelayService` instances behind **one hostname** with a token-tier switch; sponsorship decided at **mint time by the member's own policy** (three paths: member initiates / caller holds a cap / standing lookup); atproto **service-auth JWT authenticates the member's instruction** while croft-admit mints the capability the caller carries; short cap lifetime where **renewal re-evaluates policy**, so revocation lands mid-call; registry in croft-admit (not CISS, not the PDS record); an `IdIndex` seam (Transparent + HMAC-over-pair) built and tested with its security property **explicitly not claimed** pending key custody. | `raw/croft-relay-tiered-admission-fork-vs-embed-2026-08-07.md` | **preserved-verbatim (in-session dialogue)** for the dialogue; Part 0 is **preserved-condensed (cleaned-paste, §4)** for material pasted from prior sessions. No creds. **Closes a provenance gap:** Part 0 §B is the **original croft-relay build plan**, which had never been filed — `experiments/croft-relay/README.md:16` pointed at it as though it were captured. **Every mechanism claim is source-verified** against pinned `iroh-1.0.3` / `iroh-relay-1.0.3` / `CISS` / `croft-stack` (per-claim file:line table at the file's end), so this is **not** `[UNVERIFIED]` dialogue. **Two corrections to committed record:** ADR-0004's "wrap the admitted connection in a `Bucket`" fallback is **unreachable** (`RateLimited` is `pub(crate)`; embedders never touch the post-handshake stream), and the earlier claim that split instances break cross-tier reachability is **wrong** (relays don't federate; clients hold one `ActiveRelayActor` per relay, so instances federate through clients). Distilled → `plans/2026-08-07-1-plan-croft-relay-tiered-admission.md`; ADR-0006 + T62 gate rewrite pending as that plan's Phase 1. |
