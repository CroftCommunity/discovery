...
|---|---|---|
| **AppViewLite integrations · Bluesky origin retold · R2 pricing · LinkedIn/Reddit UI blueprints · feeds mechanics · Graze Social (Gemini, multiple sessions, pasted 2026-08-23)** — a batch whose value is the **owner's paste-time annotations** binding threads to projects: the LinkedIn-shape blueprint **"(stellin)"**, the Reddit-shape blueprint **"(forage)"**, the Graze tagline round (*"almost all still work after graze to forage rename"*), and the Graze Social thread the owner marks **"(led to graze rename)"** — the only documented trigger for the 2026-08-21 Graze→Forage rename (neither NAMING.md nor the clearance doc records Graze Social). Also: AppViewLite's read-only Mastodon/Nostr/RSS/Tumblr posture; the Bluesky origin story retold on the owner's substrate-thesis framing; R2 1–50 TB price points + CDN op-cost mechanics; Goodreads licensing + Inventaire; Treksky/'Gram feed mechanics, feeds-vs-hashtags-vs-lists, feed building. | `raw/appviewlite-r2-linkedin-reddit-blueprints-feeds-graze-gemini-2026-08-23.md` | **preserved-condensed (cleaned-paste, content-faithful — §4).** AppViewLite Q&A appeared **twice verbatim → reproduced once**; two threads answer **screenshots not in the paste**; threads 6/9 predate the rename. Origin story defers to **E52**'s `[UNVERIFIED]` posture — don't launder this telling; R2 pricing point-in-time volatile; blueprints' IP framing is a model's gloss, not legal advice. Distilled → COHESION **§79**; ECOSYSTEM §5m += **Graze Social** (the collision) + **Inventaire**; no new ROADMAP rows (material feeds stellin/Forage/E132). |
| **The forum-client / "incentivization resistance" dialogue · Threema architecture · cloud sync · PLC analytics · open Goodreads (Gemini, pasted 2026-08-23, dialogue date unknown)** — the heavyweight body is the owner's forum-over-Bluesky design dialogue (**owner turns verbatim**): feeds-as-subreddits, presentation-local downvote lexicon, "rotate the box", the 2–3-degree local social tree, the real-person VC credential — Forage's design lineage — culminating in the owner **coining "incentivization resistance"** (cause-side of Doctorow's enshittification) with an explicit *"note it for extraction later"* and Gemini's expanded 4-section manifesto sketch. Also: Threema's full architecture (VoIP push/CallKit, no-server-history, Mediator "message reflection", single-primary-phone, Threema Safe identity-only backup, B2B model, gateway-only interop); cloud-sync internals (FastCDC, 3-way merge, journals); PLC census tooling; open-source Goodreads. | `raw/forum-client-incentivization-resistance-threema-sync-gemini-2026-08-23.md` | **preserved-condensed (cleaned-paste, content-faithful — §4); owner turns verbatim incl. dictation glitches.** May span multiple sessions (footers mid-paste). Threema/Reddit/tool claims `[UNVERIFIED]` (messengers SoT: `research/messaging-solutions-landscape.md`); "Big Graph Service" noted as dated terminology. Distilled → ROADMAP_TODO **E132** (the extraction order), COHESION **§78** (Forage lineage; Threema ↔ recovery fork; Mediator ≈ meer custodian-queue shape; CallKit ↔ croft M4; Sifa name collision), ECOSYSTEM §5m. |
| **client-side WASM · openprices idea · Chatto/LiveKit · iroh+WebRTC two-tier relay · Blacksky/PDS mechanics (Gemini, pasted 2026-08-23, dialogue date unknown)** — six bodies: the WASM-in-PWA survey (3 rounds + libp2p/WebRTC clarification); the owner's **openprices** idea (public price log on atproto, draft `org.openprices.observation` lexicon, polymorphic IDs, web-of-trust aggregation; Open Prices/GasBuddy landscape); the Chatto/LiveKit brokered-SFU stack; the owner's **browser-calling requirement** (verbatim) + the two-tier iroh+WebRTC candidate with Gemini's own feasibility review; Bluesky/Blacksky federation, blob storage, relay crawling (requestCrawl + plc.directory `/export` polling), PDS census, full lookup walkthrough. | `raw/wasm-pwa-openprices-chatto-iroh-webrtc-blacksky-gemini-2026-08-23.md` | **preserved-condensed (cleaned-paste, content-faithful — §4); owner requirement turns verbatim.** **CORPUS FLAGS in-file:** the architecture's load-bearing `iroh-webrtc-transport` is FACTCHECK-flagged **likely nonexistent**; `enable_stun` possibly superseded by QAD; iroh-relay gating ground truth = **croft-stack's shipped admission machinery**, not the claimed `access.http`. Census/rate-limit/Blacksky figures + Chatto `[UNVERIFIED]`. Distilled → ROADMAP_TODO **E130** (openprices) + **E131** (browser tier, verification gates), COHESION **§77**, ECOSYSTEM §5m. |
| **WebAuthn-PRF key derivation · atproto clients · Bluesky native chat · Germ alternatives · Waypoints (Gemini, pasted 2026-08-23, dialogue date unknown)** — a five-thread lookup session: re-finding the derive-keypairs-from-passkeys technique (PRF/hmac-secret → salt → deterministic 32-byte secret → HKDF → arbitrary keys; legacy signature-as-entropy and why nondeterminism breaks it); atproto client/SDK landscape; mobile-app comparison; Bluesky native chat = centralized off-repo `chat.bsky.convo` over TLS, not PFS, not in the PDS; non-Germ E2EE integration paths; and **Waypoints** (`@aturi.to/waypoints`, "Open in…" client picker for `at://` URIs, from an owner screenshot not in the paste). | `raw/webauthn-prf-atproto-clients-bskychat-waypoints-gemini-2026-08-23.md` | **preserved-condensed (cleaned-paste, content-faithful — §4).** Gemini (owner-confirmed); citation chips reproduced as markers, unfollowable. **DRIFT CAUGHT:** reintroduces the FACTCHECK-refuted "atmessaging-proto working group / Bluesky integrating MLS" claim — flagged in-file, do not cite. WebAuthn-PRF baseline defers to `research/2026-07-27-social-tree-factcheck.md` (verified there). Client lists `[UNVERIFIED]`, plausibly partly invented — deliberately NOT registered; Waypoints registered as the one earned row (ECOSYSTEM **§5m**, incl. code/app-list caveat). No creds. Distilled → COHESION **§76**; no new ROADMAP row (seams reference existing items). |
| **Forage — naming the social forum site (Claude Code session, 2026-08-21)** — the naming session that settled the forum product: graze confirmed taken (the Graze bsky feed-builder co.) and Amble retired unloved → five register pivots (croft-universe water/journey words → small-town-gossip third space → .exchange compounds → creative-TLD compounds like village.green/mill.stream → tweet-test nature behaviors → corvids → domain hacks) → **DECIDED AND ACTED: Forage at forage.fyi (purchased in-session, premium tier ~$16.90/yr), `/f/<community>` paths, members = foragers**. Carries the full availability ledger (per-candidate `dig NS` sweeps + RDAP confirm), the atproto/bluesky collision sweeps (Rookery contested by rookery.network + Roost's "rookery" vocabulary; Forage clean incl. an atproto-ecosystem README grep), the USPTO knockout (both software-adjacent FORAGE marks DEAD; theforage.com mark is careers-class), and the optional corvid brand-world (ravens-as-foraging-recruiters per Heinrich; **"Nevermore" as governance-charter name** — the owner's anti-enshittification reframe). | `raw/forage-naming-social-forum-2026-08-21.md` | **user turns preserved VERBATIM** (live session, filed same-session; typos kept); **assistant turns content-faithful, NOT byte-pristine**; tool calls bracketed; the one screenshot turn (Porkbun cart) described in a bracketed note — §4. No creds. Availability statuses are a **dated snapshot** (dig-NS heuristic; only forage.fyi RDAP-confirmed then purchased). Trademark findings = knockout search, NOT legal clearance; FORAGEPRO (76357396) status `[UNVERIFIED]`. Distilled → `research/forage-name-clearance-2026-08.md`, `NAMING.md` (forum-layer Amble→Forage, supersession banner added), ROADMAP_TODO **E122**, COHESION **§75**. ECOSYSTEM deliberately untouched (Roost/rookery.network/theforage.com are name-collision context, not ecosystem peers; Graze already registered). |
| **treesocial — atproto social graph as a literal tree at tunable radii (Claude, 2026-08-21)** — founding design dialogue for a standalone atproto site: enter a handle, see follows as branches / followers as roots (mutuality = symmetry across the soil line), radius tunable outward with counts, PDS/provider indicated. Surviving arc: v3 (roots, prune preview, growth timeline, vines, PDS spotlight, silhouette export) → v4 (rings view as trunk **cross-section** — grain lines = age per quarter, bold rings = social distance; foliage = below-threshold counts, so "de-Charlie-Browning" was a semantics upgrade) → a closing ideas round (seasons-as-activity, firehose weather, **grafts**, forest, **dendrochronology**, feed-lens, sound). Owner: likely to become its own project. | `raw/treesocial-atproto-social-tree-radii-2026-08-21.md`; mocks verbatim at `treesocial-unpacked/` (4 HTML files, byte-verified; `treesocial.zip` retired with owner authorization) | **preserved-condensed (cleaned-paste, content-faithful — §4), PARTIAL:** the paste omits the exchanges that produced the first two mocks (owner: "they were not a fit," paste is all there is) — gap bracketed in-file; the mock artifacts themselves survive verbatim in the unpacked seed. No creds. Jetstream per-connection filter-limit claim `[UNVERIFIED]` (flagged by the dialogue itself). Distilled → ROADMAP_TODO **E118**, COHESION **§72**; ECOSYSTEM deliberately untouched (Jetstream already registered; mock account names are fictional). |
| **iroh-in-a-browser-extension (relay-only) + the arecipe substitution-engine plan (Claude, ~2026-08-12, pasted 2026-08-21)** — two bodies. (A) Can an iroh peer in a browser extension hole-punch? **No** — extensions share the page network sandbox (no raw UDP; `chrome.sockets.udp` died with Chrome Apps), so iroh-in-browser is **relay-only**; WebRTC is the only browser mechanism that punches, and iroh doesn't speak it (exploratory). A durable design constraint for any web/extension Croft client. (B) arecipe: PR #87's direct-regex substitution matching is too naive → a mostly-deterministic phased plan (reviewed ingredient vocabulary → pure matcher → search-by-ingredient + substitution engine → correction overlay → **evidence-gated** closed-set fuzzy tier), the included/excluded walk-through, and the external-food-DB round (USDA FDC, FoodOn, annotated ingredient-phrase sets, FoodKG) as build-time, review-gated inputs. | `raw/iroh-browser-extension-and-arecipe-substitution-engine-2026-08-12.md`; plan artifact **verbatim** (owner-pasted) at `arecipe/plans/2026-08-12-1-plan-ingredient-normalization-and-substitutions.md` | **preserved-condensed (cleaned-paste, content-faithful — §4), PARTIAL:** Body B's design dialogue between the opening question and the plan delivery is not in the paste (the plan message references its decisions as settled) — gap bracketed in-file. The **amended** plan (external-DB edits) was offered, never produced — the filed plan is the original; the edit is still to make (E119). No creds. Body A consistent with the FACTCHECK SoT; browser-sandbox + dataset-licensing specifics dialogue-cited, `[UNVERIFIED]` this session. Distilled → ROADMAP_TODO **E119**, COHESION **§73**, ECOSYSTEM **§5l**. |
| **PWA nested scope — per-game install vs whole-shelf install, and why Firefox lags (Claude, 2026-08-09)** — can each game at `fun.croft.ing/<game>/` be installable while the whole shelf also is? Mechanically yes (**one manifest per page**, so where you press install decides which app you get; distinct `id` keeps them separate), but it is Chromium's **worst-rated** nested-scope configuration: with the outer app installed and the inner not, game links do not link-capture; a TWA shelf captures everything. Safari (iOS/macOS) and Firefox have **no install prompt and no link capturing**, so the either/or resolves by whichever page you were on — scope on iOS is presentation-only. Plus a Mozilla-PWA history: the 2021 SSB removal ("little to no perceived user benefit"), the **"harmful"** standards position on File System Access, and Firefox 143's taskbar web apps as explicitly *not* full PWA support. | `raw/pwa-nested-scope-per-game-install-2026-08-09.md` | **preserved-condensed (cleaned-paste, content-faithful — §4).** Thinking-summary headers appeared **duplicated** in the paste → reproduced once; citation chips stripped. No creds. **ALL platform claims `[UNVERIFIED]`** — model-generated with unfollowable chips. **FACTCHECK does NOT bear** (it covers iOS-**P2P**; this is Safari install/web-clip behaviour) — recorded so it is not cited as though it settles them. **Grounding findings added in-session (clearly marked, not part of the paste):** `fun.croft.ing` has **no PWA wiring at all**; the dialogue's `scope: "/"` advice **violates croft-pwa's hard relative-path standard**, whose build *fails* on absolute-root paths; and the load-bearing `id`-resolves-against-**origin** question is flagged unverified rather than guessed. Owner decision: nested scope, **conditional on it working** → the `id` check is a build-plan gate. ECOSYSTEM deliberately **not** touched (browser vendors are platform substrate, not register peers). COHESION §69; ROADMAP_TODO E98 sibling. |
| **BookWyrm/BookHive open-data ladder, then the PDS→iroh endpoint lookup and croftcall (Claude, 2026-08-09)** — two bodies in one paste. (A) Book-review open data: ActivityPub **message-passing** (instances start blind) vs atproto **PDS + AppView over a firehose** (cold start disappears, identity portable); BookHive's thin arch (Hono, static HTML + Hono JSX, **SQLite/Kysely as a rebuildable AppView cache**, PDS as source of truth), lexicon space **`buzz.bookhive.*`** (`buzz.bookhive.book` w/ `hiveId` + status ref into `buzz.bookhive.defs#reading`); then the correction that **"open" is two properties** — Open Library is open by **license** (CC0 + bulk dumps, but aspect-tag reviews, not prose), BookHive open by **construction** (public records + weekly sanitized exports, but **no CC0 on review text**), ActivityPub side neither. (B) The **origin** dialogue for croft-relay (embed-before-fork; calibrate coordination bytes rather than guess), the croftcall exchange page (handle→DID→PDS→`ing.croft.iroh.endpoint/self`, deep link `croftcall://call?…`), the Android scaffold, and the monorepo/Pages build instructions (**one `docs/contract.md`**, verify-then-compile). | `raw/bookhive-open-data-and-iroh-endpoint-lookup-2026-08-09.md` | **preserved-condensed (cleaned-paste, content-faithful — §4).** **Body (B) appeared TWICE verbatim → reproduced once** (same pattern as the 2026-07-27 croft-landscape paste). No creds. The four deliverables are filed **verbatim** at `seeds/callpds-unpacked/` and referenced, not re-embedded. **SUPERSESSION recorded in-file:** B's single-record contract (rkey `self`, `getRecord`) is superseded by the tiered-admission plan's **Phase 10** (per-device records, `listRecords`) — green-lit as of `main` @ `cc94b26`; **do not implement from B**. Android client **crashes immediately** (owner-reported); the unresolved `VERIFY` markers are the leading suspicion but **not confirmed against the real crash** — recorded as suspicion, not cause. Distilled → ROADMAP_TODO **E98**, ECOSYSTEM **§5k**, COHESION **§70**. |
| **Emoji Wars / Levelforge — the three modes, then the reshape and the tier decision (Claude, 2026-08-09 → 08-11)** — Part 1 (pasted): the build plan that makes the refactor **structural** (no module reads a global world size; modes become a `GameMode` interface, not inline `isDrop()` branches), fixtures inlined, a 16-row verification matrix; then **bounce mode restored as first-class** — `meta.mode` becomes `sling \| drop \| bounce`, Red Ball-style traversal, and the discovery that the spec already had it sketched as **"drive"** (folded, rather than two names for one idea). Part 2 (live): Emoji Wars **belongs in `fun`**; player and builder are inseparable **as a product**, separable **as code** → **frontstage/backstage/lobby**; a canvas/media layer that is **a smear, not yet a layer** (9 files, ~246 refs) with the dialect **deferred until a second use case exists**; and the tier question settled by measurement. | `raw/emojiwars-levelforge-three-modes-and-reshape-2026-08-09.md` | **preserved-condensed (cleaned-paste, §4)** for Part 1; **preserved-verbatim (in-session)** for Part 2's owner turns. No creds. Deliverables filed verbatim at `seeds/emojiwars-unpacked/` (byte-verified). **Grounded against the real repo** (`CroftCommunity/levelforge` @ `e4f5adc`): build-plan Phase 2 **already done**; `main.ts` is **3,721 lines / 39% of the codebase** with **14 inline mode branches and no `GameMode` interface**; **no lobby module exists**; repo modes are `slingshot\|drop\|drive` vs the spec's `sling\|drop\|bounce`. **Owner reversal (2026-08-11):** a deterministic engine was **measured, found to work, and declined** (Rapier spike `eb70cff` — native==wasm bit-identical; the control shows *only native* moves, so `libm_force` drags native onto wasm's libm). Ratified as **`fun/docs/BUILDING-GAMES.md` §11 Tier-3** (commit `10df5fe`): *share inputs never outcomes*; *the data/sim line must be visible in the directory structure*. Distilled → ROADMAP_TODO **E99**, COHESION **§71**. |
| **croft-relay execution + the CISS storage model (working sessions, 2026-08-09 → 2026-08-11)** — the sessions that *executed* the tiered-admission plan and produced the CISS storage taxonomy. Nine-question walk-through closing every plan open question (IdIndex custody ladder, private-CISS store, two-age DID cache, git-pin standing rule, device-scoped caps — "isn't that just another endpoint ID?", croft-stack relocation, 30-day aggregate observability); plan Phases 1–4 + 6 built RED-first (gate-before-code; the airlock after the in-line decorator was **refuted at source** — post-upgrade downcast to `TokioIo<MaybeTlsStream>`; the `insert_relay` probe: **no reconnect** → disconnect-to-upgrade; budget-and-drop with zero mutation survivors; the both-processes-restart persistence test against a real pinned `ciss` binary). CISS grew vocabulary-free `kv.flag`/`kv.counter` kinds (PR #35, closed registry honored). The deletion question ("is this a 'we aren't actually deleting' issue?") became **ADR 0005**: six declared axes per stored kind (retention/authorship/erasure/enumeration/hashing×algorithm/sizing), `chain.counter` with ack-before-shred checkpoints, `kv.counter` removed-not-deprecated — then **borne out by a full-store cross-inspection** ("a much better framing than ... iterative build by use case"), now `CISS/docs/ARCHITECTURE.md` §5a. | `raw/croft-relay-execution-and-ciss-storage-model-2026-08-09.md` | **preserved-condensed** (sessions spanned compactions; owner statements verbatim; ground truth = the commit record + the plan's Review Log). Continuation of the 2026-08-07 design transcript. |
| **MLS exclusion, readmission, and the epoch-roll model (Claude Code session, 2026-08-12 → 08-16)** — began as "run the three parked experiments" and became an interrogation of whether the delivery design's model of exclusion matches what MLS does. Produced **S15–S20 (22 tests, Rung A)**, `src/outer_seal.rs`, and `Meer::sweep_with_retention`. **Confirms the base layer** (N=10 governance ban: nine survivors share new key material, the banned member derives none — measured at AEAD grade, not merely a counter check). **And finds the structural problem:** migration-to-cold (§11.6) and a ban (§11.8) are the *same* removal commit, and the thing meant to distinguish them — §11.6's *"linked by the MLS resumption mechanism"* — **cannot be built** on openmls 0.8.1 (resumption PSKs resolve from a store an external-commit group initialises empty; `add` is `pub(crate)`). So **cold and banned are mechanically the same state.** Also: re-entry is **self-admission, not a request** (nothing to deny); the window is **exactly the members whose view predates the ban**; **every `GroupInfo` carries `external_pub`** so there is no safe one and the **ratchet tree is the only dial**; refusal holds at **two** layers (keys + addressing) but **forks the group invisibly** — two branches agree on the epoch number. **Refutes one normative claim:** §11.8/Appendix-E-L6's *"zero marginal exposure"* — a re-seated member is a **full member at the current epoch**, so exposure is everything from admission to re-exclusion. | `raw/mls-exclusion-readmission-experiments-2026-08-12.md` | **user turns preserved VERBATIM** (live session, voice-dictation artifacts kept as-is); **assistant turns content-faithful, NOT byte-pristine**; tool calls summarized as bracketed notes — §4. No creds. Tagged **Drystone-spec §11**, **Drystone-delivery**, **meer-queue**. **Ground truth is the code** (`tests/s15_*.rs`…`s20_*.rs`) — where narrative and code disagree, the code wins. **Records 5 in-session corrections**, three to our own prior conclusions (S14's "neither mechanism applies"; S16's PSK assumption; the assistant's own "the spec needs to move", reversed on reading §11.8) and one to a **test that measured nothing** (the ratchet-tree arm set the group config then exported the tree anyway). **Two measurement-grade upgrades** converting epoch/counter refusals into key refusals. Distilled → `beta/drystone-spec/DOSSIER-exclusion-and-readmission-2026-08-16.md` (the findings doc, reframed as a **dial** per owner 2026-08-16), working fork `part-2-certifiable-design-WORKING-2026-08-16.md` (byte-verified at fork), `alpha/plans/2026-08-14-1-plan-readmission-and-groupinfo.md`, ROADMAP_TODO **E105/E106/E107** + **E96** updated. **Canonical `part-2-certifiable-design.md` deliberately NOT edited** — normative changes surfaced, not applied. **UPDATED at second filing (same session): S21/S22/G1 + the scenario walk + the v2 comparator appended to the raw — including the owner's no-server correction (Part 1 §2.4), the withdrawn key-1 "fails open" finding (the fold hard-stops, order-independently; E108 files what survived), and the comparator alignment (code moved to spec, versioned + rebuild-tested).** `[UNVERIFIED]`: CISS assertion-plane payload size limit (searched, none found). |
...
| CISS file-sync/dials build session (Claude Code, 2026-08-07→09) | `seeds/transcripts/raw/ciss-filesync-dials-build-session-2026-08-07.jsonl` (verbatim 11 MB session log) + `…-2026-08-07.md` (condensed work record) | **preserved-verbatim** (the session JSONL — every tool call/test/dialogue turn) + preserved-condensed companion. A *build* transcript, not a pasted dialogue: CISS PRs #18–#34, releases v0.6.0/v0.7.0 (file-sync M4/M5, relay+persistence+cost-twin wave, the **self-assertion dials** D1–D5, bilateral receipts). No external factual claims to check — the session's own artifacts (plans/ADRs/POSTURE/changelogs in the CISS repo) are the distillation. See E89/E90 stamps |
...
| Amble naming + cooperative PDS-hosting metered-billing dialogue (claude.ai, pasted 2026-07-31) | `seeds/transcripts/raw/amble-naming-coop-metering-dialogue-2026-07-31.md` | **preserved-condensed (cleaned-paste, content-faithful — §4)**. Two threads: (a) the forum pad's bare-link "wish someone would recommend to me" feature + the naming move to **Amble by Croft** (working name, **contested** clearance) + app-store lockup mechanics + the 80/20 go/no-go; (b) the **cooperative PDS-hosting metered-billing** design ("second half"). **NOT-LEGAL-ADVICE** — clearance/mark specifics dialogue-sourced. The item-storage experiment suite the metering half points at **is on `main`** (`experiments/item-storage-protocol/` + `-standalone`, PRs #33/#34/#36; `SPEC.md` = the pasted spec, cited not copied). Clearance memo → `research/amble-name-clearance-2026-07.md`. Distilled → `NAMING.md` (Forum-layer: Graze→Amble), `thinking/cooperative-social-union-model.md`, `crystallized/principles.md` ("meter the boundary"), proof-ledger. Advances **D5**. See COHESION §64/§65 |
| Graze forum — subjective-consensus karma + Reddit-parity mechanics (Gemini, pasted 2026-07-31) | `seeds/transcripts/raw/graze-forum-subjective-consensus-gemini-2026-07.md` | **preserved-condensed (cleaned-paste, content-faithful — §4)**. Gemini dialogue → atproto/iroh facts **dialogue-sourced, `[UNVERIFIED]`** (align to SoT FACTCHECK; Jetstream "MST" is atproto-repo-correct, distinct from the iroh-docs correction). Covers ungameable karma (AppView aggregation + custom downvote lexicon + Sybil resistance), **graph-weighted subjective consensus** (the social-tree thesis made concrete; Depth-0/1/2/3 + Network/Tree two-score UI), subjective Hot/Rising/Controversial, Ozone-labeler moderation, zero-cut tipping, community forking, offline archives, two-mode (Global/Subjective) no-mandatory-AppView, third-party-app feature research, titleless-post handling, Pixel Pals PWA feasibility. Named projects (atwork.place, Sifa, `place.atwork.listing`) pending-verification. Distilled → `thinking/app/` + `ECOSYSTEM.md` (flag dialogue-sourced). Bears on the E80↔E62 fork (COHESION §63). See COHESION §66 |
| Meer store-and-forward → CISS custodian queue (Claude Code session, 2026-08-07) | `seeds/transcripts/raw/meer-store-and-forward-custodian-queue-2026-08-07.md` | **user turns preserved VERBATIM** (live session, not a paste — voice-dictation artifacts kept as-is); **assistant turns preserved content-faithful, NOT byte-pristine** (diagrams/tables/corrections retained, connective prose tightened); tool invocations summarized as bracketed notes — §4. Tagged **CISS**, **Drystone-delivery**. Moves the meer from "a service with its own store" to "a thin gateway over CISS **custodian chains**": queue-only custodial write gated by a genesis-fixed **chain kind** declared in the *owner's* manifest slot (never in a head), custodial content in a separate custodian-signed record so CISS B1/B3 keep their proofs, per-DID queues as default with a **custody dial** for pooled/meer-owned, drain on **CISS account identity never MLS identity**, meter-≠-bill with the transit meter as the corpus's unmeasured **offline-data fraction**. Prior-art finding: **no MLS delivery service exists to copy** (RFC 9750 leaves the DS unspecified; OpenMLS ships a PoC, phnx-im/air is AGPL-3.0, MIMI reintroduces an ordering hub). Records **7 in-session corrections/reversals**. Distilled → `thinking/meer-as-custodian-queue.md`, `plans/2026-08-07-meer-lane.md`, `experiments/meer-queue/SPIKE-SPEC.md`. Supersedes the storage half of `thinking/meer-superpeer-design.md`. Advances Phase-10 (`plans/croft-stack/10-drystone-layer.md`) |
| CISS cost-ceilings / prepaid-meter-equity dialogue (claude.ai, pasted 2026-08-07) | `seeds/transcripts/raw/ciss-cost-ceilings-and-prepaid-meter-equity-2026-08-07.md` | **preserved-condensed (cleaned-paste, content-faithful — §4)**. **Begins mid-conversation** (opening user turn not in the paste; not reconstructed). Tagged **CISS**. Extends the §65 meter-the-boundary mechanism with the *ceiling* dimension: synchronous metering ⇒ hard cost cap; throttle-not-debt; exit unconditionally cap-exempt; the prepaid meter as equity-vs-poverty-trap. External facts **model-asserted `[UNVERIFIED]`** — McCarthy/Waldrop *Dream Machine* quote wording, EU Data Act dates (Sep 2025 / 12 Jan 2027), UK Ofgem/British Gas figures (3.2M/2022, £20M fine, Jul 2023 cap-equalization), AWS no-hard-cap/egress-waiver claims; the *design implications* are the durable takeaway. Distilled → `thinking/cost-ceilings-and-the-prepaid-meter.md`, `crystallized/principles.md` (2 Tier-3 bullets), ROADMAP_TODO **E89**/**E90**. See COHESION §67 |

## Code — verbatim confirmed

Each proof/experiment tree was `git clone`d from its croftc upstream-repo PR branch and
copied unchanged. `diff -rq` against the branches is **empty** (only the added
PR-CONVERSATION/CODING-TRANSCRIPT files and excluded upstream-repo plumbing differ). Code
...
The five croftc/upstream-repo PRs (#3, #4, #6, #8, #9) remain the canonical upstream for
...
|---|---|---|
| **`croftc/upstream-repo#10` code** — Rust Cargo workspace: `crates/{core,bluesky,cli,design,shell,web,desktop}` + as-built `BUILD-SPEC.md`/`design-philosophy.md` + real recorded Bluesky fixtures + ~20 PNG web snapshots | **preserved-verbatim (code)** | Placed at `experiments/croft-app-phase0/` from head branch `claude/experiments-pcl2ym`; verified **byte-identical** (`diff -rq`, empty). Only the PR's `experiments/` subtree imported; upstream-repo root `.github/`/`Publish/`/`README.md` excluded. No secrets/build-artifacts (`.gitignore` excludes `/target`, `crates/web/dist/`). Fixtures are real recorded public `getTimeline` responses (no fabrication). |
...
| `spec.zip` — 19 drystone RUN briefs (RUN-01…13 + RUN-08/09 combined/remainder), CLASSROOM-ARC/DIAGRAMS, F1-amended-R7-text, THE-FOUR-GATES, RESEARCH-PROMPT-recovery | `seeds/spec-unpacked/` (`.zip` retired) | **preserved-verbatim** (byte-verified). The RUN briefs' executions are on main (RUN-*-SUMMARY + the spec-reconciliation F1–F8 landed RUN-02/03/05). RESEARCH-PROMPT-recovery → its report is filed (`research/e2ee-recovery-postmortem-…`). **THE-FOUR-GATES.md** is genuinely-new (the feasibility-verdict companion: I9 = decision, resolution-ACL = design, X1 = purchase, release pass = calendar) — frozen here; a **candidate `thinking/` working copy** (owner-gated, not placed). |
...
## 2026-07-22 intake — ponds roadmap / account kernel / ENS framework / client-side-search research (Phase 1)

A batch of four pasted sessions (three claude.ai design dialogues + one Gemini research/game/cards
session), filed as five raw files under `raw/` (Gemini session split from the ENS naming chat along its
natural seam). Phase-1 filing only (raw + manifest + connective tissue + `beta/OPEN-THREADS.md` staging);
folding into beta layer docs and authoring the ponds roadmap are Phase 2, gated on a between-phases
discussion. Plan: `../../plans/2026-07-22-transcript-intake-ponds-kernel-ens.md`.

| Raw artifact | Where | Status |
|---|---|---|
| **Card-maker webxdc packaging + push notifications (claude.ai, 2026-07-22)** — greeting card = portable artifact vs the maker = hosted PWA; three-rung output ladder (image / single-file HTML / `.xdc`); collaborative card needs a hosted URL + `.xdc`-as-enhancement; the untrusted-renderer domain-separation pin; carry-static-not-derive; Pushover-for-ops-only vs Web-Push-for-recipients (iOS 16.4 + Home-Screen caveat) | `raw/croft-card-maker-webxdc-packaging-and-push-2026-07-22.md` | **preserved-condensed (cleaned-paste, content-faithful — §4)**. UI chrome (doubled thinking-summary headers, "Added to memory") stripped; no creds. Web-Push/iOS 16.4 + Pushover $4.99/10k-mo figures `[UNVERIFIED]` (product-level). An orphaned opening line preserved with a note. Weaves into the kernel raw. |
| **Encrypted prefs record + repo-mirror + the `account.croft.ing` kernel (claude.ai, 2026-07-22)** — `ing.croft.account.prefs` ciphertext record synced via the PDS; prefs as the training-wheels case for encrypt-in-public-repo; the local mirror re-derives a PDS (getRepo CAR / getBlocks / listRecords + Sync-v1.1 causal-log + local CID/sig verification + CID-keyed blockstore); `repo-mirror` as one bought-once primitive; the shared `account.croft.ing` origin = session broker + prefs cache + repo mirror + write outbox = one kernel; subdomains become stateless skins | `raw/croft-encrypted-prefs-repo-mirror-and-account-kernel-2026-07-22.md` | **preserved-condensed (cleaned-paste, content-faithful — §4)**. UI chrome stripped; no creds. **`io`/`omg` in source are typos for `croft.ing`** (user-confirmed; normalized in distill, noted in raw). Uses "Merkle Search Tree" for atproto's repo — **FACTCHECK-confirmed correct** (the refuted claim is iroh-docs=MST, which this does not make); Sync v1.1 FACTCHECK-aligned. Largest net-new body (the kernel is thinnest-covered in the corpus). |
| **Games-pond roadmap + browser-native P2P + phased build (claude.ai, 2026-07-22)** — the `fun.croft.ing` ponds-roadmap ask; browser-native P2P for WASM games ("iroh but browser-native"); the P1–P10 phased build plan (P10 Sustainment as a standing drill; determinism-first; verifiable clean-clear via move-list replay vs state hash; follow-chain-not-global leaderboard); solitaire-P1-first sequencing | `raw/croft-games-pond-roadmap-browser-p2p-phased-build-2026-07-22.md` | **preserved-condensed (cleaned-paste, content-faithful — §4)**. UI chrome stripped; no creds. **Truncation note:** the assistant's answer to the browser-P2P question is not in the paste (known answer: WebRTC/matchbox, already in `../../beta/cairn/iroh-app-pond-building-blocks.md`) — flagged answered-elsewhere. |
| **ENS backronym — multi-vantage framework (non-Claude, 2026-07-22)** — enshittification → polite ENS shorthand; the three-lens framework (Business = Extractive Neglect Syndrome / Systems = Eventual Non-utility State / User = Erosive Nudge Syndrome) wrapped by the recursive FOSS-tradition "ENS is Not Service"; the copy-paste write-up prompt preserved as the durable artifact | `raw/ens-backronym-multivantage-framework-2026-07-22.md` | **preserved-condensed (cleaned-paste, content-faithful — §4)**; framing prose condensed, all candidate expansions + user decisions preserved. **Status: CANDIDATE THINKING, not settled** (user decision) — staged in `beta/OPEN-THREADS.md`, NOT written to `NAMING.md`. Ties to `beta/fenced/aggregation-theory-and-the-enshittification-shield.md` + `beta/socialization/coop-messaging-research.md` (already flags the acronym-re-expansion move). |
| **Gemini: atproto client-side search + Heardle-shaped music pond + Kudoboard (Gemini, 2026-07-22)** — serverless-SPA client-side search architecture (Jetstream / CAR-parse via atcute / FlexSearch·MiniSearch·Orama·uFuzzy·SQLite-FTS5-WASM / BM25 / OPFS VFS matrix / COOP-COEP + coi-serviceworker / multi-tab locking); Heardle-as-ENS-cautionary-tale; a local-first PWA music-guessing pond (`app.heardle.result` lexicon, audio-adapter layer, licensing models A–D, the 6-channel "sounds-like" pastiche starter set, crypto-monotonic/CID anti-cheat, guest-first, per-backend challenges, hidden-issues catalog); Kudoboard explainer | `raw/gemini-atproto-clientside-search-heardle-pond-kudoboard-2026-07-22.md` | **preserved-condensed (cleaned-paste, content-faithful — §4); WHOLE-DOCUMENT `[UNVERIFIED]`** — Gemini output the user flagged "needs more fact-checking"; every version/bandwidth/benchmark/limit/browser-behaviour/library-name claim is to be verified at outcome-processing time (Phase 2). UI chrome + "Opens in a new window" stripped; citation list folded into a Sources block. **Closes the noted gap** at the `arecipe.zip` row above ("truncated 'Architectural Blueprints…client-side search' research … NOT filed — needs the full text"). Uses "Merkle Search Tree" for atproto — FACTCHECK-confirmed correct. |

**Carried-forward + open decisions (surfaced, not resolved — the user's calls):** the untrusted
card-renderer **domain-separation** answer is a *tentative conclusion* (separate registrable domain,
revisitable) → `beta/OPEN-THREADS.md` gate; the **music-game licensing** question (pastiche closeness;
Nintendo/SEGA/JASRAC) is **backlog + candidate thinking only**, not a resolved gate; the **ENS framework**
is candidate thinking; the Gemini research **fact-check** runs fully at Phase-2 outcome time. Connective
tissue updated this intake: `COHESION.md` (new section), `ROADMAP_TODO.md` §E (kernel, ponds roadmap,
Heardle pond, research fact-check, Web-Push onboarding), `ECOSYSTEM.md` (Kudoboard, Pushover, Web Push,
client-side search engines), `beta/OPEN-THREADS.md` (all threads staged). Nothing folded into beta docs
yet (Phase 2).

## 2026-07-23 batch — socialization + arecipe + a new measurement helper

Seven transcripts filed in one session (framed as "socialization mostly," but mostly arecipe / client-
architecture). Two socialization pieces were **distilled to beta this pass** (user decision); the rest are
**Phase-1 (raw + connective tissue)**, with fact-check of the many volatile technical claims deferred to
Phase 2 — **except the SSO transcript, web-verified this pass** (the user asked). The uncommitted 2026-07-22
batch is left untouched. Plan:
`../../plans/2026-07-23-transcript-intake-socialization-arecipe-measurement.md`.

| Raw artifact | Where | Status |
|---|---|---|
| **Hansel & Gretel → the "Gilded Weaver" fable (Gemini, 2026-07-23)** — H&G as an enshittification teaching-tool analogy (Doctorow); Great-Famine origins; the 4-phase pedagogical framework (pebbles-vs-breadcrumbs = durable open standards vs platform shortcuts; white duck = open protocols/ActivityPub); psychopomp/water-boundary lore; cross-cultural deceptive-trap tales (Pinocchio/Pied Piper/Rumpelstiltskin/Jorōgumo/Painted Skin/desert ghoul); big-tech-as-shapeshifter; the villain-name iteration Azure→…→**Gilded Weaver / "Weaver of the Golden Hall"**; advertising-as-benign-but-crushing-in-aggregate; the white-duck-logo (distinct from DuckDuckGo) branch | `raw/hansel-gretel-enshittification-gilded-weaver-fable-2026-07-23.md` | **preserved-condensed (cleaned-paste, content-faithful — §4)**. Chrome stripped; **source paste duplicated the H&G thread → reproduced once**; video/image prompts kept verbatim. Folklore-scholarship claims `[UNVERIFIED]`, low-stakes. **Distilled this pass** → `beta/socialization/the-gilded-weaver-fable.md` + `logo-white-duck-psychopomp.md`; Gilded Weaver + white-duck logo = **working candidates at beta tier** (OPEN-THREADS T61). COHESION §54. |
| **Bluesky / AT-Protocol autonomy-origin story (Gemini, 2026-07-23)** — Twitter→Bluesky (Dorsey 2019) → Jay-Graber-autonomy → PBC-spinout → Musk/X (2022) → AT Protocol as open infra buildable at "~40M-user scale without reinventing the wheel"; user portability + decentralized moderation + infra-at-scale | `raw/bluesky-atproto-autonomy-origin-story-2026-07-23.md` | **preserved-condensed (cleaned-paste, content-faithful — §4)**. Chrome stripped; no creds. **Origin dates + "~40M scale" `[UNVERIFIED]`, dialogue-sourced** — FACTCHECK does not cover them; ECOSYSTEM already records `Bluesky PBC` as verified. **Distilled this pass** → `beta/socialization/the-autonomy-origin-of-the-substrate.md`; **Jay Graber → `kindred-work.md`**. COHESION §55; verify at Phase 2 (ROADMAP_TODO). |
| **account.croft.ing SSO — storage-vs-cookie & browser support (Claude, 2026-07-23)** — the storage-based mock SSO is Chromium-only (K1); cookie+BFF for cross-browser (KC1); DPoP-bound sessions; single-origin-core + isolated-subdomains | `raw/croft-account-sso-storage-vs-cookie-browser-support-2026-07-23.md` | **preserved-condensed (cleaned-paste, §4); source paste duplicated → once. Web-VERIFIED this pass** (user asked). Records the **corrected** mechanism: WebKit partitions storage by top-level subdomain (**same-party**, not third-party); the DPoP key is a **non-extractable** WebCrypto `CryptoKey` in IndexedDB (can't ride a cookie); cross-browser SSO ⇒ **server-set** cookie + BFF (a `document.cookie` cookie is ITP-capped at 7 days). Folds into the uncommitted 2026-07-22 kernel thread. COHESION §56. |
| **arecipe — CNAME/paths, PWA keyboard, camera+AI, LLM import, timers, seasonality (Claude, 2026-07-23)** — CNAME can't carry a path (RFC 1035); PWA keyboard shortcuts (reserved keys; `keyboardLock` fullscreen-only; manifest `shortcuts` = jump list not bindings); camera + on-device-AI meal-suggestion dreaming; LLM import = **model-selects-never-writes + verbatim-substring invariant**, deterministic-JSON-LD-wins; timer = **absolute end timestamps**; seasonality = **boost-only, toggleable** | `raw/arecipe-cname-pwa-keyboard-camera-ai-import-timer-seasonality-2026-07-23.md` | **preserved-condensed (cleaned-paste, §4); WHOLE-DOCUMENT technical claims `[UNVERIFIED]`** (Phase-2 fact-check). arecipe-repo design thinking. **3 RUN files delivered as `timers.zip` — CAPTURED verbatim** at `seeds/timers-unpacked/` (byte-verified 2026-07-23). Fact-checked (E61): all CONFIRMED. COHESION §57. |
| **arecipe — Wikibooks recipe-corpus import & static precache (Claude, 2026-07-23)** — Wikibooks `{{Recipe summary}}` sparse map; `categorymembers` authoritative over the ToC; pipe-trick ingredients = link-parse; ~3,600 pages; CC BY-SA 4.0 (June 2023) / 3.0 stamping; Action API over deprecated dumps; `$wgRCMaxAge` 30d kills recentchanges for a 6-mo rerun; static precache via atproto `getLatestCommit`/`getRepo since rev` (CAR/MST), bundle-as-cold-start-cache-never-authority | `raw/arecipe-wikibooks-corpus-import-and-static-precache-2026-07-23.md` | **preserved-condensed (cleaned-paste, §4); quantitative claims `[UNVERIFIED]`** (Phase-2). Uses "Merkle Search Tree" for atproto — **FACTCHECK-confirmed correct**. **3 RUN files delivered as `wiki.zip` — CAPTURED verbatim** at `seeds/wiki-unpacked/` (byte-verified 2026-07-23). Fact-checked (E61): 2 corrections (no `{{Recipe summary}}` `category` param; pipe-trick not mandated) fold into the importer. COHESION §57. |
| **arecipe — empty-tile chip (mobile vertical real estate) (Claude, 2026-07-23)** — pictureless tiles as an inline chip at single-column (media zone at multi-column); the 3:2-vs-16:7 ratio bug; D0 discovery gate; TDD red-first | `raw/arecipe-empty-tile-chip-mobile-2026-07-23.md` | **preserved-condensed (cleaned-paste, §4) for the dialogue; RUN-EMPTY-TILE-CHIP preserved VERBATIM** (pasted in full). Design/UX, low fact-check; ratio measurements eyeballed `[UNVERIFIED]`, low-stakes. A design-comparison HTML artifact was produced (not captured). COHESION §57. |
| **A privacy-preserving usage-measurement kit (Claude, 2026-07-23)** — **edge counter over a declared graph** (counters not logs; `nav.a__to__b`); registry-generates client+panel+fixtures; disclosure panel double-duty (rich local view vs exact outbound counts); `expires` honored at runtime; Poplar/VDAF heavy-hitters noted-and-skipped (needs 2 non-colluding operators); CNIL 2025 three-event ceiling | `raw/usage-measurement-edge-counter-privacy-kit-2026-07-23.md` | **preserved-condensed (cleaned-paste, §4) for the dialogue; RUN-MEASURE-01 (E0–E8) preserved VERBATIM** (pasted in full). **A NEW cross-property helper concept** (arecipe = recommended pilot); quantitative claims (sendBeacon 95.8%/82.9%, 64 KiB cap, CNIL 2025, Litestream ~2.6M PUT/mo) `[UNVERIFIED]`, Phase-2. Ties to Skylite's sponsor-vs-child retention stance + the existing Caddy/SQLite/Litestream/R2 kit. COHESION §58. **Proof-out EXECUTED as arecipe PR #58** (`measure-proof/`, 56 files, 48 unit + 4 e2e green — RUN-MEASURE-01 E0–E8; OPEN). |
| **"Social Tree" — Reddit/forum + Google-Reader-RSS lens PWA over Bluesky/atproto (Claude, 2026-07-27)** — one long product/UX dialogue: default bsky can't sort-by-comments/time → a client-side PWA can; forum-idiom mapping; Rust/WASM client engine; long-form via `site.standard.document` + comment-anchor threads; RES/Apollo power-feature restoration; the **degree-based radius model (r=0/r=1/r=2/r=∞)**; quiet local mutes + private local user-tags; **RSS-reader-as-1-tap-discussion-seed** unification; riding native browser bookmark/`storage.sync` sync as transport; credential/VC/passkey integration; the **utility-vs-extraction** thesis (finite feeds, intentionality budgets, continuous local export, zero-take economics, ethical WebRTC-relay tier + Privacy-Pass metering); "PWA is the floor, native follows"; the user's tenet **"machines compute provenance, never utility"**; a 5-phase roadmap | `raw/bsky-reddit-forum-social-tree-pwa-2026-07-27.md` | **preserved-condensed (cleaned-paste, content-faithful — §4).** Light chrome stripped; no creds. **FACT-CHECK DONE (not carried as `[UNVERIFIED]`)** → `research/2026-07-27-social-tree-factcheck.md` (~40 load-bearing claims; unusually accurate; **5 REFUTED** — no native atproto E2EE/private-data (E2EE is third-party Germ/MLS), no `app.bsky.feed.hideReply` method (use threadgate `hiddenReplies`), no native threadgate "mutuals" rule, Privacy Pass = RFC 9576/9577/9578 not 9152, `PublicKeyCredential.supportsPrf` isn't real; plus Chromium-only/iOS-Safari platform caveats). `site.standard.*` real-but-community. **This is E39 (aggregator pond) walked into product** — extends, not net-new. Distilled → ROADMAP_TODO **E62**, ECOSYSTEM §5i, COHESION §59; re-derives invariant S5. |
| **Croft landscape — small/med/big-world positioning · tree-as-relational-UI · did:webvh watchers · blob-dedup↔E2EE · co-op service menu · bsky clients · auth/MFA · Lightning/Nostr · responsive filters · Rust (Claude, 2026-07-27)** — a long compound session: the small-world positioning thesis; the OPFS/continuous-export honest correction; tree-as-social-UI research (information-scent reframing + 10 design criteria); did:webvh versionTime/watchers + federation-of-availability; Bluesky blob per-DID scoping ↔ the client-side-E2EE storage contract; the co-op low-maintenance service menu; best Bluesky clients (→ borrow for Skylite); passkey/DPoP/DBSC + flat-cost TOTP MFA + JWKS mass-invalidation; Lightning/Nostr zaps for community payments; the arecipe responsive-filter playbook; Rust single-binary/musl | `raw/croft-landscape-small-world-tree-ui-webvh-blob-coop-2026-07-27.md` | **preserved-condensed (cleaned-paste, content-faithful — §4).** Opening third was **triplicated in the paste → reproduced once**; long in-line research bodies condensed with numbers/claims retained; no creds. **FACT-CHECK DONE** → `research/2026-07-27-social-tree-factcheck-2.md` (~40 claims; unusually accurate; corrections: iOS-18 hotspot client-IPv4-isolation UNVERIFIED/forum-only; Riseup pads 60d; DBSC shipped Chrome 146; popover Baseline-Newly-not-Widely; Path-50≠Dunbar; MIMI=WG-draft). Distilled → ROADMAP_TODO **E63–E71**, ECOSYSTEM §5j, COHESION §60. |
| **atproto mechanics deep-dive (PDS/relay/AppView/blobs/auth/rev/inversion) + phone-hotspot P2P (Claude, 2026-07-27)** — two paired Q&A exchanges: (1) two phones over one phone's Wi-Fi hotspot (host↔client easy; iOS-18 client↔client IPv4 isolation caveat; Local Network permission); (2) a heavily citation-backed how-atproto-works-at-Bluesky explainer (account lifecycle + status propagation; OAuth [PDS=resource-server, AS may be a separate entryway]; service-auth JWT; blob two-phase-commit + GC; firehose + requestCrawl; rev discipline; prevData/MST inversion "inductive firehose"; proxying; size limits; getRepo=current-state-only) | `raw/atproto-mechanics-pds-relay-appview-hotspot-2026-07-27.md` | **preserved-condensed (cleaned-paste, content-faithful — §4).** Committed to main by external process (HEAD 7556a51) ahead of this manifest row. **FACT-CHECK DONE** → `research/2026-07-27-social-tree-factcheck-2.md` (size limits 2 MB/1 MB/200-ops/5 MB ✓; relay 2 vCPU/12 GB/30 Mbps ✓; PDS_CRAWLERS=bsky.network ✓; getRepo current-state-only ✓; iOS-18 hotspot IPv4-isolation UNVERIFIED/forum-only). **Deeper superset of** `atproto-architecture-appview-relay-explainer-2026-06-22.md` — distil into one atproto reference, don't duplicate. COHESION §60. |
| **PWA/SPA cross-origin content-fetch (extension model) · "Graze by Croft" forum naming · Skylite→Bluebird rebrand (Claude, 2026-07-29)** — continuation of the 07-27 social-tree/forum body. Thread A: how a zero-backend static PWA reaches arbitrary reader/API content the same-origin policy blocks → **decided as the browser-extension model** (not a user-run local proxy), **Playwright spike greenlit**. Thread C: forum working name **Graze by Croft**, `/f/`, commons framing, herd-puns-at-edges; `forum.croft.ing` now / `graze.ing` later. Thread D: **Skylite → Bluebird** ski-beginner rebrand (Lodge/Trail Map/runs, Cabin Mode, trail-rating = **trust distance not maturity**, "Bluebird for Bluesky" lockup); **RUN-BLUEBIRD** transformation (no-skylite-string test + banned-vocab lint; lexicon NSIDs w/ trust-tier enum while record-free; oauth client-metadata URL = silent breaker; frozen-seed provenance). | `raw/croft-pwa-content-fetch-extension-graze-bluebird-2026-07-29.md` | **preserved-condensed (cleaned-paste, content-faithful — §4).** Italic thinking-summary chrome + trailing "Run bluebird plan / Document · MD" marker stripped; long assistant passages condensed with claims retained; no creds. **SCOPE REDACTION:** an unrelated **an unrelated work repo** GitHub-App/PAT + commit-attribution thread was **EXCISED** (out of Croft scope) with an in-file `[EXCISED]` marker — not a fidelity loss. `[UNVERIFIED]` (spike/clearance, not carried as fact): localhost mixed-content exemption, Chrome PNA rules, Safari stance (→ E72 spike settles); Bluebird lockup clearance (Amex card / Bluebird.js). atproto/social-tree claims defer to the 07-27 FACTCHECK docs. **Ground truth:** `CroftCommunity/bluebird` + `bluebird.croft.ing` live (rename DONE); local `skylite/` remote stale. Distilled → `NAMING.md` (Forum-layer + Skylite→Bluebird), ROADMAP_TODO **E72–E74**, COHESION §61; forum-plan reconciliation note. |
| **Client-side / static-hostable browser games catalog (Gemini, 2026-07-30)** — a broad hunt for games that run **100% client-side** (WebGL/WASM/JS) and serve as **static files, no backend**: TuxRacer.js + the Emscripten/WASM porting ecosystem (SM64/Quake/Doom/OpenRCT2), a large arcade/platformer/physics/puzzle inventory (SuperTuxKart, HexGL, Slow Roads, Sandspiel, VVVVVV, Celeste-PICO-8, OpenTyrian, 2048, Sokoban, …), the **bubble-shooter / Puzzle Bobble / Frozen Bubble** family, a kids/educational subset (Tux4Kids, Scratch+TurboWarp, Blockly, GCompris), emulator-hosts (Infinite Mac, JS-DOS, Internet Archive embeds), the Emscripten-toolchain explainer, and a copyright aside (95-yr rule; Paperboy→Warner Bros.; Archive DMCA exemption) | `raw/client-side-static-browser-games-catalog-2026-07-30.md` | **preserved-condensed (cleaned-paste, content-faithful — §4).** "AI generated" image captions + citation chips stripped; user questions near-verbatim, Gemini answers content-faithful; no creds. **A NEW single-player/static axis** distinct from the two 2026-06-21 P2P/social catalogs; maps to the as-built `fun.croft.ing` shelf. **ALL repo/license/liveness claims Gemini-sourced `[UNVERIFIED]`** (per-candidate confirmation is the inclusion gate); legal aside flagged for fact-check; no atproto/iroh/iOS claims → FACTCHECK doesn't bear. Distilled → `thinking/app/ponds/client-side-static-game-candidates.md`. **Drove the owner charter decision** (two-tier shelf, COHESION §62) + two `fun/plans/` docs (bubble-shooter Tier-1 build; TuxRacer.js Tier-2 wrap spike). ROADMAP_TODO E46 (appended). |
| **Behavior-scale methodology + Stellin & Graze build sessions (Claude, 2026-07; renamed 2026-07-31)** — two claude.ai design dialogues: (A) LinkedIn UX research → the methodology (titled "The Behavioral Twin" in-session; renamed **behavior-scale** 2026-07-31) + the **Persona-Switch Prototype** method + the **Meridian**/Stellin build prompt (prof-networking mock, `stellin.app`); (B) Reddit/Slashdot UX research → the topic-aggregation build spec + the **Graze** persona-switch spec (forum mock, `graze.ing`). Establishes the reusable build pipeline (seed research → extended spec → branded build prompt → milestone build) and the mock↔large-tier continuum. | `raw/stellin-graze-behavior-scale-sessions-2026-07.md` | **preserved-condensed (cleaned-paste, content-faithful — §4).** UI render chrome stripped (collapsed thinking-summary headers, "Document · MD", "Show more", "All sources gathered"). The two original UX-seed descriptions were **truncated in the source paste** → kept as **truncated pointers**, not reconstructed. The large deliverables are filed **verbatim in their working homes and referenced, not re-embedded** (per §4 no-triplication): methodology `thinking/behavior-scale/{behavior-scale-methodology,persona-switch-prototype}.md`; build specs `thinking/app/build-specs/{stellin-meridian-build-prompt,graze-persona-switch-spec,graze-topic-aggregation-build-spec}.md`; research `research/{linkedin-ux-architecture,reddit-aggregation-ux-slashdot-baseline}-2026-07.md`. No creds. Distilled/wired → `thinking/behavior-scale/README.md`, ROADMAP_TODO **E80** (+ E73 update), COHESION **§63**, ECOSYSTEM §5c-3, NAMING (Stellin + Forum-layer). |
| **croft-relay — tiered admission, fork-vs-embed, member-sponsored tunneling (Claude Code, 2026-08-07)** — the design session that followed RUN-CROFT-RELAY-03/PR #40. Walks the owner's opening "fork iroh-relay on a maintained branch" instinct to a **no-fork** design, then — after a Pass-2 gap analysis and a full feasibility sweep — to a materially different one. **Settled design:** *introduction is free, carrying is what membership buys.* Two independent gates — a **cap** (may you call me; the callee's choice) and **membership** (will we carry you; commercial). Enforcement is a **byte budget with a clean disconnect**, not rate-limiting — the owner's objection that throttling-until-media-fails "feels ugly" was correct, and the budget change **deleted the tier switch** (one relay instance, no rate limit, a supervisor that drops). We ship **our own binary depending on the unmodified crate** — not a fork, and the only shape in which a counting decorator can exist. Caps are **opaque-id records in the callee's own repo**; per-device lexicon records (rkey = label, `listRecords`); a **user-chosen** request-to-token policy (`anyone | mutuals | nobody`); revocation is **both** expiry and grant-record deletion. Storage shrinks to membership + accounting — **no social graph persists anywhere**. | `raw/croft-relay-tiered-admission-fork-vs-embed-2026-08-07.md` | **preserved-verbatim (in-session dialogue)** for the dialogue; Part 0 is **preserved-condensed (cleaned-paste, §4)** for material pasted from prior sessions. No creds. **Closes a provenance gap:** Part 0 §B is the **original croft-relay build plan**, which had never been filed — `experiments/croft-relay/README.md:16` pointed at it as though it were captured. **Every mechanism claim is source-verified** against pinned `iroh-1.0.3` / `iroh-relay-1.0.3` / `CISS` / `croft-stack` (per-claim file:line table at the file's end), so this is **not** `[UNVERIFIED]` dialogue. **Corrections to committed record and to in-dialogue claims:** ADR-0004's "wrap the admitted connection in a `Bucket`" fallback is **unreachable** (`RateLimited` is `pub(crate)`; embedders never touch the post-handshake stream); the claim that split instances break cross-tier reachability is **wrong** (relays don't federate — clients hold one `ActiveRelayActor` per relay, so instances federate through clients); **"you can boot someone mid-call" is wrong** (`on_connect` runs once, token expiry disconnects nothing — eviction needs `Clients::disconnect` actively driven); and **per-member byte accounting is not obtainable from iroh** (metrics are server-wide counters), which is why the counting decorator exists. Distilled → `plans/2026-08-07-1-plan-croft-relay-tiered-admission.md` (comprehensively rewritten 2026-08-08; Review Log retains the superseded Pass-1/Pass-2 records). ADR-0006, the ADR-0004 mechanism supersession, and the T62 gate rewrite are that plan's Phase 1. **Phase 1 executing (2026-08-10):** the workspace moved to `croft-stack/relay/source/` (crate renamed `croft-relay-admit`), croft-stack gained its gate workflow, and T62's gates were rewritten. **Also reads `CroftCommunity/connect`** (the lexicon + exchange page + Android client), previously unreferenced in this corpus. |

**Phase-2 resolved (2026-07-23):** the six RUN files are now **CAPTURED verbatim** — `seeds/timers-unpacked/`
(RUN-TIMERS-SEASONALITY, RUN-GUIDE-HELPER, EXP-IMPORT-EXTRACTION) and `seeds/wiki-unpacked/`
(RUN-WIKIBOOKS-CORPUS, RUN-RECIPE-META-STRIP, RUN-BUNDLE-PRECACHE), byte-verified against `timers.zip`/`wiki.zip`
(the archives, at the CroftC root outside any repo, can be retired at the user's discretion). The **fact-check
is DONE** (`../../research/2026-07-23-batch-factcheck.md`, ROADMAP_TODO E61): mostly CONFIRMED; REFUTED = the
RUN-MEASURE-01 sendBeacon rates (real ~82% unload / ~92% load); PARTLY = the 64 KiB Beacon-spec framing, the
`{{Recipe summary}}` `category` param, the pipe-trick mandate, and the Bluesky "~40M" (= registered accounts).
The **measurement proof-out ran as arecipe PR #58.** **Retention decided** = bounded + visible clear control.
**Still open (the user's):** the measurement kit's **name and home**; **rounding vs exact counts** (gated on
E8); reviewing PR #58's findings. Connective tissue this intake: `COHESION.md` §54–§58; `ROADMAP_TODO.md` §E
(E52–E61); `beta/OPEN-THREADS.md` T61; `ECOSYSTEM.md` §5h; `beta/socialization/kindred-work.md` (Jay Graber);
the two beta socialization distillations; and `research/2026-07-23-batch-factcheck.md`.
