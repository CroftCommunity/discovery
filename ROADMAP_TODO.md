...
| A8 | **Import Croft-app Phase 0 (CroftC PR #10) → `experiments/` + the IP/ownership decision** — built in a CroftC repo; moving it onto chasemp/CroftCommunity infra with a clean paper trail is the move, but the call is the user's (invention-assignment / CoI vs the Head-of-Product-Security role). Most time-sensitive. | [decision] | `ROADMAP.md:157` | `thinking/app/README.md:49`, `COHESION.md:322` |
...
| E6 | **Games pond** — P2P games over iroh, outcomes as a custom lexicon (rendered-where-desired), Bluesky as the rendezvous channel; the hook that teaches composability; outcome-attestation is the real design work (set aside). Standalone research prompt staged, unrun. | [explore] | `thinking/app/design-philosophy.md:634` | `seeds/generated-prompts/games-pond-research-prompt.md`, `COHESION.md:322` |
