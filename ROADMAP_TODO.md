...
| E19 | **The two-CLIs question / croft-chat-cli rework** — `experiments/iroh/crates/croft-chat-cli` (iroh/P2P group-messaging CLI, TDD/production) vs `experiments/croft-app-phase0/crates/cli` (Bluesky-pond feed CLI, Crux-style core/shell). Decide whether croft-chat-cli adopts the Phase-0 functional-core/imperative-shell + planes discipline; relates the "graduate experiments → project repos" question. Also: the `iroh/` experiment is **not in the `experiments/README` index** (close that gap). | [explore] | `COHESION.md` §23 | `experiments/iroh/crates/croft-chat-cli`, `thinking/local-first-as-design-imperative.md` |
