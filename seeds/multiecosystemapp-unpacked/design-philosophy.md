...
## 1a. The organizing thesis: a user-respecting utility garden

Composability is itself the user-respecting value. Not a feature, a stance.

The app does not decide how useful any given pond is to a given user. The user
decides, by shaping their own garden. A pond that matters to one person is
absent and unmissed for another, and the design must let it be present-and-useful
for the former and absent-without-feeling-incomplete for the latter. "Useful or
not to a given user" is a property of the design working as intended, not a
failure of it.

This reframes what a pond is. A pond is not a feature shipped to everyone. It is
a capability that can be present, and the composability means presence is
per-user, per-need. The UI guarantees (consistency, warmth, low mental load,
follow-or-ignore) are the constant soil quality. The ponds are what the user
chooses to grow. The soil stays good no matter what is or is not planted.

This is the idea the architecture has been converging on all along. Modules
behind ports, capability declaration, honest seams, and a shell that composes
whatever is available were all, without being named as such, building toward "the
user assembles their own garden from available ponds." Naming it makes it the
organizing thesis rather than an emergent accident.

### The garden test for any future pond

Does it add optional leverage a user can adopt or ignore, without the surface
feeling broken either way?

- If yes, it is a candidate. It can be tried cheaply (one module behind a port)
and kept only if it earns its place. The garden tolerates a bed that never gets
planted.

- If it only works by being forced on every user, it is against the grain,
regardless of how good the feature is in isolation.

A consequence worth stating: abandoning a pond that does not earn its place costs
nothing structural, because it was always just one module behind a port. This is
what makes it safe to try speculative ponds without violating "grow by use case",
the experiment is cheap and reversible by construction.

The garden test gates whether a unit should exist. A separate document,
`design-criteria.md`, gates how every pond and pad must behave and look once it
does (the shared criteria, the pond-specific and pad-specific additions, and the
visual system). The two kinds of composable unit: a pond is a connection to an
external social ecosystem (native data, honest seams, attribution); a pad is a
small self-contained app that runs in the shell (a game, a tool; sandboxed,
permission-scoped, vetted). Ponds are ecosystems; pads are the lily pads that
float in the garden.
...
- **Credit and traceability.** The surface is opinionated and consistent, but
nothing is hidden in the sense of concealed or claimed as ours. Every pond's
content carries honest attribution to its origin, every borrowed technique or
upstream project is credited, and a curious user can always follow the thread
down to the source. We present one clean thing without ever pretending it is the
whole truth or that we made all of it. See section 4a.
...
## 4a. Credit and traceability (follow or ignore)

This is the ethical companion to honest seams. Honest seams says different things
are shown as different. This says: the smoothing we do for the eye must never
seal the seam against the curious hand.

The principle has two halves, and both are mandatory:

- **Ignore is the default.** The surface is opinionated, consistent, and enough
on its own. A non-technical user never has to know or care where a post's data
physically lives, which protocol it rides, or what is underneath. Low mental
load is the point.

- **Follow is always available.** A curious user can always peel the surface
back: click through to the origin, see which pond and which service a piece of
content actually comes from, learn what substrate is underneath. The path down is
real, not a dead end. Hiding the seams smooths them for the eye; it never welds
them shut.

Opinionation is about presentation, not about obscuring provenance. We present
one clean thing, but we never pretend the clean thing is the whole truth or that
we built all of it.

This coheres with decisions already made, and is in part just the explicit
statement of an ethic they already implied:

- Pinning holds an address, not a copy, so a pinned item still points at its real
origin.

- Ponds stay native rather than fused, so every piece of content can be traced to
where it actually lives and behaves.

- We credit what we build on. The architecture sits on others' work (the Crux
pattern, iroh, the atproto service-proxy and service-auth mechanisms, published
recipes from other projects for consuming PDS-signed tokens) and says so, rather
than presenting itself as wholly original. Attribution to upstream projects and
techniques is a standing practice, not an afterthought.

A note on borrowed techniques versus ponds: crediting a project for a technique
we learned from it (for example, another service's published recipe for atproto
cross-app authentication) is distinct from integrating that project as a pond.
The former is an attribution we make freely. The latter is a deliberate decision
that requires assessing whether the thing is actually a social experience worth
presenting, and is not implied merely by having borrowed a technique from it.
...
out of it entirely. The core never holds, calls, or awaits the port. (Phase 0 may
take a written-down shortcut on *where* the adapter is wired, but never on the
core calling the port; see the roadmap.)
...
our youngest dependency on the load-bearing mobile-push path, where the entire
plugin ecosystem assumes Tauri.
...
the retry paths. The invariant is proven in build spec section 3a.
...
### Candidate ponds (not committed)

These pass the garden test (section 1a) as worth trying, but are not committed.
They are recorded with their thinnest-useful-slice and open questions so a future
decision rests on this reasoning. Each can be tried as one module behind a port
and abandoned at zero structural cost.

**CANDIDATE - OpenMeet (or similar), read-only.** Thinnest useful slice: a
read-only view of the user's own meets, easy to surface or share alongside the
rest, explicitly not attempting admin or write operations. Fits the garden model:
useful to someone who uses meets, cleanly absent for someone who does not. Open
questions: whether it is genuinely a social experience worth presenting (requires
assessing the product, not just that it shares an atproto auth technique we
already credit), and exactly what data it exposes for read. Note: we credit
OpenMeet as a technique reference (its published cross-app auth recipe)
regardless of whether it ever becomes a pond; the two are separate (see 4a).

**CANDIDATE - direct P2P social games over iroh.** A social game whose live play
runs peer-to-peer over iroh (reusing the Croft substrate, confirming iroh
generalizes beyond chat) while its social layer lives on Bluesky. Composes from
other ponds rather than being self-contained: the first pond built from other
ponds' capabilities. Stays honest to public Bluesky.

The data-layer design, which is the elegant part: the entire game is played over
the iroh peer connection and stays there, ephemeral, never written to any repo.
Only the settled outcome (result, final score, whatever the players agree the
game produced) optionally becomes a custom lexicon record in a player's repo.
That outcome record is public and traceable in the open network (anyone can
resolve it, it is cryptographically attributable) but not feed-rendering
(timelines render `app.bsky.feed.post`, not a game lexicon), so it appears only
where a client knows to render it, for example a leaderboard. This is
follow-or-ignore expressed at the data layer: the outcome is open to anyone who
follows the thread to the repo, invisible to anyone without a reason to render
it. It dissolves the false binary of "public post (feed pollution)" versus
"hidden over iroh"; the third path is "public outcome record that renders only
where desired."

The durable-vs-live line is clean and the same for every game regardless of
tempo: live play is always over iroh and always ephemeral (no moves are ever
persisted); only the settled outcome is durable, and only by the players' choice
to write it. There is therefore no PDS-write-rate concern, because the durable
layer sees one record per completed game, not one per move. The result is a
public leaderboard of direct peer interactions, openly attributable and rendered
where desired: verifiable public results without exposing the private play, since
nobody can reconstruct the game from the outcome and nobody sees the outcome
unless rendering the leaderboard.

The genuine open question for this pond is outcome attestation, not write rate. A
single player writing "I won" to their own repo is attributable but unilateral,
and a leaderboard of self-reported wins is gameable. So the real design work is
how an outcome becomes trustworthy: both players attesting (two records or a
co-signed one), the iroh session producing something both peers signed that is
then published, or accepting self-report for low-stakes social games. The
directness and privacy of the peer interaction is exactly what makes the outcome
need a trust mechanism, because no server witnessed the game. This is the
subtlety to design deliberately when the pond is built. Likely mechanism (set
aside for now): a mutually-signed attestation, both peers sign the settled
outcome with their persistent identity, published as the record, verified by
checking two signatures against two DIDs. This is the "basic smart contract"
feel without a chain. Note it defeats unilateral lying but not collusion; the
latter is a social-layer concern (reputation, rate limits, or tolerated for
low-stakes fun), not a crypto one.

Prior art and design lessons (homage to Delta Chat). Delta Chat shipped this
exact pattern in production: small web apps (webxdc) shared peer-to-peer through
a chat, with realtime collaboration over iroh via a joinRealtimeChannel() API.
Lessons borrowed, distinct from the attestation question:

- Rendezvous via the social channel. Delta Chat hands off the iroh topic and
ticket through the encrypted chat so peers connect without slow global discovery.
Our equivalent: the Bluesky challenge (post or custom lexicon record) carries the
iroh rendezvous info, so the social pond is the discovery layer. No own discovery
infrastructure needed.

- Lazy connectivity. Start iroh only when play begins, not when a game is merely
available. P2P bootstrapping is expensive enough that laziness matters.

- IP-exposure constraint. Direct P2P reveals peers' IPs to each other. Delta Chat
makes this user-disablable (relay-only fallback). For strangers matched off a
public challenge this is a real privacy consideration; likely relay-only by
default for non-mutuals, direct as opt-in, consistent with the not-extractive and
user-respecting values.

- Identity split. Live play uses ephemeral iroh identities (forward-secret,
private, borrow directly); any published outcome uses the persistent atproto
identity (durable attribution). The bridge between them is where attestation
lives.

Porting leverage. The webxdc Quake III port documents a reusable recipe: take a
WebRTC-based multiplayer browser game, polyfill the browser WebRTC API to use the
iroh-backed realtime channel as transport, mock the signaling/WebSocket handshake
with server/client roles, and remove other direct-internet dependencies. This
generalizes: any WebRTC-data-channel browser game is portable by swapping the
transport, which widens the candidate pool far beyond webxdc-native games to the
whole universe of WebRTC browser multiplayer games.

Curation policy: vetted by default. The webxdc catalog is open-submission and
hobbyist-dominated, with high quality variance and some apps requiring heavy
downloads or awkward onboarding. So we do not expose a store. We surface a small,
hand-picked set vetted to our standard (instant start, no tracking, works first
time for a non-technical user). Vetting covers code quality, onboarding
smoothness, and the not-extractive guarantee, not just whether it runs.

Hook strategy. The games-plus-Bluesky combination is a candidate onboarding hook
that teaches the composable model by being it: a user sees a challenge on a
normal Bluesky post, taps, is instantly playing a friend peer-to-peer, and the
result posts back, having unknowingly watched ponds compose (Bluesky social
layer, iroh transport, custom lexicon outcome, shared chrome). This works only
with instantly-legible social games (tic-tac-toe, connect-four, word games), not
impressive-but-heavy ones (the Quake port is the wrong hook: a great ceiling
demo, a terrible first impression). The launch goal is a few genuinely polished,
instantly-understood games, including kid-friendly options (which speak directly
to user utility and the no-extractive-use-case win), plus at least a few that
adults genuinely love and find fun. Quality and fewness beat openness and
quantity at launch.

Open fork on the webxdc spec. Adopting the webxdc format (or at least its
realtime-channel API shape and the WebRTC-transport-swap porting pattern) buys the
porting leverage and commons-credit alignment with Delta Chat, without obligating
us to inherit its catalog. Resolution lean: adopt the open format for leverage and
credit, curate the surfaced set for quality. Decide deliberately when the pond is
built.

Both candidates reuse existing infrastructure categories (custom lexicons plus,
for games, the iroh substrate) and add no new structural weight, which is why
they pass the garden test.
