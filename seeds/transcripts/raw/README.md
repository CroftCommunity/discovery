...
- **Code:** verbatim. Each proof/experiment tree was `git clone`d from its croftc
  upstream-repo PR branch and copied unchanged; `diff -rq` against the branches is empty
  (only the added PR-CONVERSATION/CODING-TRANSCRIPT files and excluded upstream-repo
  plumbing differ).
