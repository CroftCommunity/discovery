...
| `the-recurring-inversion.md` | The unifying civic pattern: take an extractive stateful intermediary → reduce it to stateless/content-blind/optional → wrap it in a reinforcing institution, applied at five named scales (relay→stateless rendezvous; relay→optional superpeer; routing server→content-blind carrier; ad platform→consumer-side broker; compellable operator→cooperative). The through-line that ties five separate design moves into one non-domination argument; names Masnick's "Protocols, Not Platforms" (2019) as the naming source for the protocol-layer posture. Mechanisms cross-referenced, not restated. *(Tranche-1 recovery.)* | why five design moves are one argument |
