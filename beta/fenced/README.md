...
| `platform-dominance-and-adoption.md` | The competitive behavior of the fenced field: how the centered platforms achieve and hold dominance (Discord zero-friction onboarding, network-effect lock-in, the S-1 economics), the social-platform lifecycle as a descriptive pattern (the enshittification arc as a shape), the field map and the universal trade, and the descriptive adoption dynamics (why incumbents win, only-Signal-crossed-the-chasm). The harm reading is routed to `../activism/`; the go-to-market strategy to `../socialization/`. |
| `group-chat-failure-modes.md` | The descriptive map of how existing group-chat systems fail: the failure-mode taxonomy (survivor-determinism, covert-ordering, genesis amendability, the churn-fold Achilles heel, device-loss recovery, the identity gap) and the Matrix E2EE operational lessons (the UTD invariant, mandatory-recovery onboarding, the promised-vs-delivered expectation-gap). Drystone's answers are cross-referenced to the spec (§7, §11) and impl, not reproduced; Matrix-the-system is catalogued in `../cairn/`. |
| `app-store-survivability-and-abuse-posture.md` | The gatekeeper dimension of the fenced field (the app stores): the *Rave trap* (Apple's Aug-2025 removal; the antitrust reading) and the governance-posture escape — decouple control/media planes, no public discovery by default, hardcoded block/report/kill-switch, edge-AI moderation. The load-bearing line: "app stores don't care if your transport is decentralized; they care if your governance is." Media transport mechanics stay in spec §6.12. Facts [confirm]. *(Phase-1 recovery.)* |
| `aggregation-theory-and-the-enshittification-shield.md` | The market-economics analysis behind the open-protocol bet: Aggregation Theory (Thompson) + enshittification (Doctorow) coupled at the switching-cost hinge → a zero-switching-cost open protocol is structurally enshittification-resistant (exodus, not lock-in). Takes the switching-cost data from `platform-dominance-and-adoption.md` as input and argues the consequence. Dialogue-sourced framings flagged. *(Phase-1 recovery.)* |
