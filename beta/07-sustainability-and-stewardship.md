...
candidate) precisely so its neutrality survives stewarding non-Croft things. The through-line: the ethical
choice and the durable choice keep turning out to be the same choice — and each guard is a *structural
property*, not a behavioral promise.
...
**Establishes:** non-extraction has *two* concrete carriers, not a slogan — a cooperative mechanism (the
economic/governance answer) and a three-layer IP-stewardship structure (the anti-capture lock) — and both
defer entity overhead via phasing while making capture structurally impossible rather than merely promised.
The verified failure lineage shows each guard answers a real, documented death.
