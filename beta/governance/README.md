...
**What this layer is.** Layer 4 of the beta layer-cake: the *why-this-ownership-form* argument.
Where `drystone-spec/` (Layer 1) is the protocol and `impl/` (Layer 2) is the reference build,
this layer holds the structural case for the ownership and funding form a peer-relationship
platform must take. It is the durable home the beta plan reserved for **theme
`07-sustainability-and-stewardship.md`** (which migrates here in a later pass) and for the
peer-standing → cooperative-form argument filed below.
...
**Human-facing companions live in Layer 6** (`../socialization/`): the metaphor essay
(`tilling-the-soil-essay.md`) and the sixty-second pitch (`sixty-second-pitch.md`) present this
argument for non-technical and adoption-facing audiences. The five documents were produced as one
cross-referencing set; the split follows the layer plan (analysis here, presentation in
socialization).

**The empirical companion lives in Layer 7** (`../activism/`): this layer carries the
*structural / legal* register of the indictment (peer standing, corporate law, why a cooperative);
the activism layer carries the *empirical* register (the five-plank harm narrative, the 14-gap
evidence brief, the reference index). The Drystone spec's **Part 1 §2.6** (voice requires
field-integrity) points at *this* document for structural grounding and at `../activism/` for
empirical grounding. As of batch four (2026-07-06) `peer-standing-and-the-cooperative-form.md`
expanded (555→820 lines) to fold that empirical grounding into the argument.
...
- Two grounding gaps the argument flags as not-yet-sourced: the Rochdale / ICA cooperative legal
  ...
  capital-formation literature. Grounding these is the next work if the set goes forward.
