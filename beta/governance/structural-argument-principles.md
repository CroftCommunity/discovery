# A Structural Incompatibility Between Securitized Platforms and Peer Standing

### A principles-driven argument
...
3. The defensible definition is therefore by **symmetric right**, not by absence of roles:
   equal standing means no participant holds a capacity over another that is not reciprocal.
   This survives momentary asymmetry of behavior; it is a claim about the rule set, not the
   runtime.

4. *Equality* here cannot mean sameness. Two distinct things cannot be symmetric across every
   axis. This is the relational-egalitarian move (Anderson): equality is a relation of
   **standing**, not a distribution of identical attributes. We borrow the shape of the concept,
   not its politics.

---

## II. The locus of the wrong is capacity, not conduct

5. Pettit's non-domination: an agent dominates another to the extent it holds the **capacity**
   to interfere arbitrarily, *whether or not it ever exercises it*. The benevolent master who
   never interferes still dominates, because the other lives at his mercy. The test is the
   standing capacity, evaluated on the power graph, not on behavior.
...
7. The bridge is Pettit's own later work. List & Pettit, *Group Agency*: a structured group
   *is* an agent, constituted by its decision architecture. So a structure-backed corporate
   ...
   architecture, so it does not depend on anyone being benevolent.**

---

## III. Coherent group agency concentrates power

8. List & Pettit's impossibility result: no judgment-aggregation function satisfies all four of
   universal domain, collective rationality, **anonymity** (equal weight to each member), and
...
## IV. The securitized form's dominant pursuit

10. **[premise]** The publicly-traded securitized form makes return-to-external-capital its
    **structurally dominant pursuit**, because the share is a freely tradable residual claim,
    continuously priced, held purely as a pecuniary interest by a constituency detached from
    use.
...
13. The benefit-corporation variant (Delaware DGCL Subchapter XV) **fences but does not abolish**
    the engine: it permits directors to weigh other interests (§365(a) balancing), but owes the
    benefit-class no enforceable duty (§365(b)), lets only ≥2% / ≥$2M shareholders sue to enforce
    the balance (§367), and reports biennially to stockholders (§366). It adds a permission; it
    leaves the tradable residual claim intact.

---

## V. The domain mismatch

14. The form fits **goods and services**, where extraction is *aligned* (more use-value drives
    more extraction), *separable* (the good's quality is independent of governance standing), and
    *exitable* (the transaction completes and releases you).
...
18. **Resolution:** behavioral optimization makes relationship-shaping *possible*; the
    securitized form makes it *structurally preferred* and binds the entity *against* fixing it,
    because engagement is the measurable proxy that serves the dominant pursuit and
    stated-preference alignment is exactly what costs engagement. The mismatch is not "extraction
    always shapes badly" but "the form is structurally bound to resolve the
    alignment-versus-engagement competition toward the pursuit that shapes the relation."
...
20. Therefore shaping the field toward attention-maximization is not a side effect of a flawed
    proxy the platform might fix; it is the **product specification**. The revealed-preference
    "trap" becomes the *correct instrument*, because attention is revealed-by-behavior and
    attention is the product. The platform has no structural reason to close the gap between what
    you do and what you value, because what you do is what it sells.
...
22. **[scope]** "The user is the product" is specific to the *ad-funded* model. Subscription,
    fee, and marketplace models securitize without selling attention; the argument against those
    runs through IV-V, not VI. The attention-farming claim is the sharpest blade precisely
...
27. This closes the loop with (1)-(4). A peer is, by definition, the one whose perspective
    ...
    field's life "outside of what it yields" is not under-weighted; it is **invisible**. It does
    not show up as small. It does not show up.
...
28. **Net:** the form cannot *constitute* peer standing — only treat participants well at
    ...
    its occupants. *They can only farm us, and we are not a field.*
...
30. **[premise, qualified]** "It must start that way to continue." Not a logical necessity but a
    path-dependence claim: conversion *into* the securitized form is incentivized and easy;
    conversion *out* is near-impossible, because once tradable claims exist they create a
    constituency with legal standing to enforce the dominant pursuit (*eBay* is the proof). You
    cannot reform back to peer standing from inside, because the reform requires the consent of
    the constituency whose standing-advantage it removes.

31. **[tension]** The absolute "only the cooperative" overstates: foundation-ownership,
    perpetual-purpose trusts, and steward-ownership protect non-extractive purpose without the
    cooperative form. The surviving claim: the cooperative *dissolves* the edge rather than
...
*Epistemic note. Claims marked [premise] are structural definitions being advanced, defensible
but not sourced findings. Claims marked [tension] are genuinely contested or unresolved and are
