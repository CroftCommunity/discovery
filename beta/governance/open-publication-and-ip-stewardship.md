...
- **Reference code: Apache-2.0.** Chosen over MIT/BSD deliberately. Apache-2.0 carries an **express patent grant** plus a retaliation clause; MIT and BSD are silent on patents. For a protocol, that patent grant is the part that matters. **[decision-gate — conflicts with the sibling manifestation doc, the user's call.]** `foundation-cooperative-and-sustainability.md` licenses the reference implementation **AGPL-3.0-or-later + DCO** as a "structural-openness lock," which is the opposite posture (network copyleft, no proprietary forks) for the same artifact. The two cannot both hold for the neutral reference implementation: Apache-2.0 maximizes protocol *spread* (permissive, patent grant, proprietary forks allowed); AGPL-3.0-or-later maximizes openness *lock* (copyleft, no closing the source). A14 recorded "Apache-2.0 reference-code license" but the manifestation doc's copyleft argument is load-bearing there, so the intended split (neutral reference impl vs the Croft product/app code) is unresolved. Tracked as ROADMAP C13; do not treat either as settled until the user decides.
