...
- **Reference code: AGPL-3.0-or-later + DCO.** (Decided 2026-07-09, C13; supersedes the earlier Apache-2.0
  choice A14 recorded.) The neutral reference implementation is **network-copyleft**, not permissive: it may
  be taken, run, changed, and forked, but the source can never be closed, which is the "structural-openness
  lock" the manifestation doc (`foundation-cooperative-and-sustainability.md`) argues for. The patent
  concern that had motivated Apache-2.0 is preserved: **AGPL-3.0 (the GPLv3 family) carries its own express
  patent grant (§11)**, so choosing copyleft does not give up the patent protection — it adds a
  no-proprietary-fork guarantee on top of it. The tradeoff accepted deliberately: less permissive *spread*
  (a proprietary re-implementation cannot lift the code) in exchange for a hard openness guarantee on the
  reference implementation. The Croft product/app code is AGPL-3.0-or-later + DCO on the same basis; the
  spec *text* remains CC0 1.0 (below); the `hpke-rs` substrate dependency is MPL-2.0 (A1, mandatory).
