...
| `drystone-design/redb-storage-contract.md` | The concrete redb local-storage engine contract under the abstract authoritative-vs-derived pattern: MVCC single-writer/many-reader, constant-time savepoints for late-event re-fold, `auth_`/`idx_` table families, fold-as-sole-writer via injected traits, blobs in iroh-blobs. The load-bearing caveat: no published Jepsen/linearizability crash-safety evidence (a standing risk). Gives open-thread T25 its home. Feature facts [confirm]. *(Phase-1 recovery.)* |
| `ios-background-execution-and-the-ble-caution.md` | The iOS background-execution model (the SLC / BGAppRefresh / BGProcessing / silent-push+NSE wake-hook taxonomy mapped to iroh wake-pulses; Rust-core + Swift-shell via UniFFI) and the load-bearing negative result — spontaneous off-grid BLE meshing is aspirational (CoreBluetooth does not wake on a new advertiser; Berty's OS kills background P2P), which is *why* the push-node and meer exist. iOS facts cite the FACTCHECK SoT. *(Phase-1 recovery.)* |
| `the-four-property-tension.md` | The engineering-tension argument that licenses a chosen, revocable, blind Delivery Service (the meer + push-node): group moderation + multi-device + PFS + offline-mesh cannot all coexist without a privileged peer or degrading one property. The honest correction — not a hard "impossibility" but "no production deployment," with the serverless-MLS escapes still drafts/PoC. Prior art lives in `../cairn/object-capability-and-decentralized-mls-prior-art.md`. Deployment claims [confirm]. *(Phase-1 recovery.)* |
