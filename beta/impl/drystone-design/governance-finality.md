...
- **Authority to contribute a governance signature is per-epoch, not genesis-only.** A signature counts toward a threshold only if its author holds standing *in the current epoch*, that is, is a current member of this branch, and not merely because the author appears in an immutable genesis admin set. A departed or evicted admin's signatures MUST stop counting toward thresholds; the check MUST NOT be satisfied by genesis-membership alone. This is a distinct correctness requirement, not a restatement of the ceiling (A3): the ceiling ends a *removed member's* read and enforcement authority, while this rule governs which signatures assemble a quorum in the first place. A naive implementation that admitted any signer in the genesis admin set let a removed (or compromised-then-evicted) admin retain full governance authority forever, contradicting its own stated invariant. Scoping the check to current-epoch standing is naturally partition-local: an admin removed on the left branch loses authority there while still governing a right branch that never booted them, consistent with the two-valid-groups resting state after a fork (A9). `Verified` (admin authority is per-epoch, an evicted admin's signatures stop counting, Proofs Phase 2.6 A2.4).
...
Rationale: this is the correction the Stage 1 analysis surfaced. Resolving a slot by "the causally-maximal single fact" (R1 as first stated) is right for *which* concordant decision wins, but a decision is not a single fact; it is the k-th corroborating fact crossing the threshold. Folding a lone vote into a change would make any single member a center, which the peer-symmetry principle forbids (authority-and-complement.md). The fold resolves *assembled quorums* by R1, not individual votes. `Design.`
...
- The interval is therefore a window in which a governance-revoked member may still decrypt but MUST NOT have any action honored. This is safe under the retained-copy floor (asset-keying.md): revocation protects the future, not the past, and a bounded window in which a just-revoked member can still read content it would have seen anyway is within the existing threat model.
...
- **Fork and exit** is the floor (authority-and-complement.md §5): always available, for when a persona cannot accept the group's shape at all.
...
There is no single correct configuration, because respecting variety and canonical local state means different groups genuinely want different thresholds. What exists is not right-versus-wrong settings but better-and-worse *matches* between a group's temperament and its configuration, with all the mechanisms safe in every configuration. This is the social-layer expression of the governance-concentration spectrum (authority-and-complement.md §4): the protocol provides a safe range and lets groups choose their point in it.
...
A3 and A7 canonicalize concurrent heads by the R1 tiebreak, which fold-semantics.md leaves as FactId versus a canonical composite. Any total order identical on all nodes satisfies correctness, so this is a preference inherited from the fold-semantics open item, not a new question. `[confirm.]`
