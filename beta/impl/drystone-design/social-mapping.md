...
`Companion to: p10-drystone-asset-keying.md (the per-scope content keys and store flavors this doc's records are sealed and classified by) and p10-drystone-history-durability.md (the MLS transport, per-author chains, and content-blind store that carry private records here). Also companion to p10-drystone-atproto-ecosystem.md, which widens this document's prior-art section to the full ATProto ecosystem. Also companion to p10-drystone-part1-principles.md and p10-drystone-part2-mechanics.md; vocabulary inherits from the conventions-and-decisions reference.`
...
- **Companion linkage.** Records here are sealed under the per-scope asset keys of p10-drystone-asset-keying.md and carried by the transport and stores of p10-drystone-history-durability.md. The private-overlay distribution topologies (§H) are a specialization of the history-durability tier model. Repoint against Part 2 §5.7 (k-of-n membership) and §7.6 (fork) when this material is folded.
