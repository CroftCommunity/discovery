...
- **Two competing RuleChange quorums — FALSIFIED (refutation).** Two concurrent conflicting RuleChanges on the same rule, each carrying a valid k-of-n quorum, **auto-resolve order-dependently** (order1 → `add_member_threshold=9`, order2 → `5`; both `fork="clean"`, no hard-stop) — a silent I5 violation on exactly the shape §7.6 says must escalate. The fold's contradiction predicate set (mutual-expulsion / removed-then-included / role-thrash) has no competing-RuleChange predicate. Pinned as a refutation (`two_competing_rulechange_quorums`, register `competing-quorum-autoresolve`); the **fix is design-gated** (which competing-quorum shapes escalate) and was NOT decided autonomously — options A/B/C written to backlog §2a.
- **Per-act approver-role granularity — DEFERRED (design decision).** Step 5.6 counts approver personae by lineage **regardless of role**; whether acts should require a per-act approver-role floor is an undecided trust-model call. Options written to backlog §2a; not implemented (stop rule b).
