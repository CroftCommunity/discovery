...
The v1 and v2 briefs test the governance fold's conflict-resolution semantics (R1 through R4 in p10-drystone-fold-semantics.md): given a set of facts, does it fold order-independently and causally correctly. This v3 brief tests the layer specified on top of that in p10-drystone-governance-finality.md: how a k-of-n decision becomes effective (quorum-folding, A1), how a removal bounds a member's authority (the ceiling, A3), how the decision is enacted (the enactment dial, A4 through A6), how a node knows its authority view (the now, A7, and the fail-closed finality gate, A8), that a ban and a voluntary fork are one primitive (A9), and how genuine disagreement escalates rather than being adjudicated (B1).
...
- Real versus reference fold, exactly as before. No production fold exists in `croftc/upstream-repo`, so unless one now exists, you are testing a faithful reference implementation.
