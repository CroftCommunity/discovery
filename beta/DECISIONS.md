...
- **Settled vs walked.** Section 1 is decisions already made (with the date decided), including
  **load-bearing spec design decisions** that index down to their `drystone-spec/` home (not every spec
  choice — only the cross-cutting ones a reader would look here for, e.g. the governance model, the persona
  vocabulary, and the general delegate primitive). Section 2 is the
...
| **The general delegate primitive** — recovery, escalation, C2, failover, and revocation are **one buildable mechanism**: a `(predicate, sealed-payload)` pair with trigger / threshold / attribution knobs, bounded by the capability-vs-authority split (the courier-vs-agent cut, "liveness is the boundary"). | decided (Design-stage; unifying shape) | — | `drystone-spec/part-2-certifiable-design.md` → §5.5.1 "The general delegate primitive: one mechanism under recovery, escalation, C2, failover, and revocation" (per-case wire encodings + the recovery trust predicate are Appendix B `[gates-release]`) |
