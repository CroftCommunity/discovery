...
**Commitment.** A **peer is the representation of a human** in the system, not a node or a device; rights
and weight attach to it *because* a person is behind it. Peer-equality is therefore **equality of humans as
represented**, and it holds as long as the human-to-peer binding is one-to-one, a binding that is a
**social-utility judgment the group makes, never one the system can attest** (the razor of §2.0 applied to
identity). There is **one kind of peer**, and "equal" is made precise by asking *in what ways may one
...
- **Weight**: how much a **peer** *counts* in governance. **Flat: one per distinct peer (by lineage)**, and
...
- **Resource**: what a **node** *has* (storage, uptime, reachability, a radio). Intrinsic to the
  ...
  more. This is a fact about every node in the system, not only a peer's devices: meers and relays have
  resources too. It appears here because, *across peers*, it is one of the two legitimate inequalities.
...
governance image of the equal right, the flat count of one per distinct peer (by lineage) that no role grant
...
distinct peers by lineage, never clients. *(Part 2 §5, §7.)*
...
> re-key path can strand a peer's `tenure`; and the precise **communal-namespace key construction** for a
