...
| 7.6.2 | re-plant **message** continuity: in-flight conversation survives the repoint, no loss/dup | `Modeled` | loopback / B1 hash-structure grade; the four claims re-asserted over **real iroh-gossip** at loopback (RUN-12); test-only record serialization, no wire pinning; real-NAT = X1 | `e12_2_message_continuity.rs` (RUN-09, 5 tests: pre-repoint exactly-once, in-flight causal-order, cross-order digest equality, dup/drop detection); `iroh_message_continuity.rs` (RUN-12, 2 tests: the four claims over real gossip + fault injection) | — | `[gates-release]` B1 record encoding (App-B); real-NAT = X1 |
...
| 7.6.9 / 7.3.3 | horizon checkpoint as a foldable **fact**: corroboration count over independent identical folds | `Design` (spec unchanged); fact-form corroboration determinism earned (`Modeled`) | fold-side fact shapes; test-only serialization, NOT the release encoding; §7.3.3 semantics unchanged (co-signature corroborates, never substitutes) | `horizon_checkpoint.rs` (3 tests, RUN-12 EXP-H2: corroboration deterministic across members/orders; no false corroboration; open contradiction persists across successive checkpoints) — reuses `horizon_checkpoint::{CheckpointFact, corroboration_count}` | — | `[gates-release]` §7.6.9 manifest encoding (App-B) |
...
| 6.8.1 | steady-state anti-entropy: a frame lost between connected peers is detected + repaired without reconnect, folds re-converge | `Modeled` | loopback; whole-set `(device,lamport)` range-compare stand-in | `steady_state_anti_entropy.rs` (RUN-09); `anti_entropy.rs` (range-summary/diff) | — | — |
| 6.8.1 | range-**partitioned** production form: a large divergent range repaired in O(log)-ish rounds shipping only the divergence (Negentropy-style 1-d recursive reconciler over `(device,lamport)`) | `Modeled` | loopback; test-only additive-monoid fingerprint, `[gates-release]` wire fingerprint (App-B) unpinned; real-NAT/real-transport = X1 | `partitioned_anti_entropy.rs` (RUN-12 Part 3b, 3 tests: diff-only equivalence, fingerprint composition, O(log) rounds at scale); recommended in `beta/impl/drystone-design/rbsr-construction.md` (Part 3a) | — | `[gates-release]` wire fingerprint (App-B); real-transport loss = X1 |
