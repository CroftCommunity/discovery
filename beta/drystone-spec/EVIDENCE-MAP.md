...
| 5.2 / 5.10 (group-principal seam, §2e) | Group principal = communal namespace `H(tag ‖ group_id)`; personae = self-authorizing subspaces; capability issuance downstream of the folded Group Role, **re-issued not revoked-in-place** across a fold-driven authority change (stale cap fails, re-issued succeeds, deterministic on both members) | `Design` | model only — no trust tier (I9), no MLS internals, test-only serialization; the `SubspaceId` byte encoding stays `[gates-release]` (E.1) | `group-principal-seam/tests/seam.rs` (RUN-11 Part 3, 5 assertions) | — | `[gates-release]` `SubspaceId`/genesis-id encoding (App-B, E.1); `green-real` upgrade FINDING-stopped by the Design scope wall |
