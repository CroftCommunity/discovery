...
| Resolution threshold | [charter] | any value ≥ 1, dialed by governed RuleChange; **no protocol floor** — a below-2 setting is a declared attribute (consented-to danger); a contested subject signs but never counts — Part 2 §7.3.2 | **2** ("no one-signature verdicts") | `Measured` (E117 P1/P4 pins; owner decisions 2026-08-21/25) |
| Readmission threshold | [charter] | any value ≥ 1 — gates the readmission DECISION that clears a ban's standing ceiling (distinct from the add dial: inviting strangers ≠ undoing a ban) — Part 2 §11.8 | **1** (easy group, easy mercy — permissive like the invite dial; "two to un-ban" is one dial away) | `Design` (dial); ceiling + fact-vs-decision asymmetry `Measured` (E117 P4) |
| Token issue/revoke weights | — | **not a dial.** Issue like adding (add threshold), revoke like removing (remove threshold); revocation naming no issuance refused loudly — Part 2 §11.7 | mandatory pairing | weights `Design`; refusal `Measured` (E117 P4); owner decision 2026-08-25 |
| Exit voice | [persona] | **silent** (author nothing; liveness machinery carries the roster consequence, standing intact) · **voiced** (self-authored departure fact, quorum-exempt, zero group authority, optional leaver-chosen metadata) — Part 2 §7.6.4 | leaver's sole choice, per exit | kind distinction + exit floor `Measured` (E117 P4); metadata extension `Design` |
