...
## §6.8.1 range-partitioned production form landed at loopback (RUN-12 Part 3b)

`Status: complete (RUN-12, 2026-07-16)`

Scope: §6.8.1's "range-partitioned production construction open" residual is closed at loopback grade. RUN-12 Part 3a read the two candidates (Willow 3d-range, Negentropy) against §6.8.1's linear `(device, lamport)` key space and recommended the Negentropy-style one-dimensional reconciler (brief `beta/impl/drystone-design/rbsr-construction.md`); Part 3b built it. No mechanism-of-truth change; the grade is `Modeled` at loopback (A.9 when-in-doubt).

**§6.8.1, done.** The two residual clauses ("the specific construction is not yet chosen"; "still the open §5 / Appendix B choice") now name the recommended construction and the landed partitioned form: a Negentropy-style one-dimensional recursive reconciler over `(device, lamport)` — count-balanced sub-ranges, an additive-monoid range fingerprint, an `IdList`/`Fingerprint` mode switch — repairs a large divergent range in O(log)-ish rounds shipping only the divergence, replacing the whole-set compare. It is `Modeled` at loopback because the fingerprint is test-only (the `[gates-release]` wire fingerprint, Appendix B, is unpinned) and real-transport loss is the X1 residual. A clarification is recorded: the governance-fact reconciliation surface (§6.8.5/§7.2) is a distinct Willow-shaped 3d key space and remains a separate choice — not a contradiction. The open-threads §6.8.1 entry and the EVIDENCE-MAP §6.8.1 rows carry the same landing.

**Verification:** no em-dashes or double-hyphens in this changelog prose; the Part 2 inserts use house em-dash style; the grade is `Modeled` (loopback), when-in-doubt per A.9; cross-references (the brief path, §6.8.5, §7.2) resolve on the site build; the broken-ref gate plus the emitted-HTML anchor audit stay clean. Evidence: `alpha/experiments/croft-chat/croft-chat/tests/partitioned_anti_entropy.rs` (RUN-12 Part 3b, 3 tests green, RED→GREEN via a neutralized-fingerprint probe), `beta/impl/drystone-design/rbsr-construction.md` (Part 3a), §6.8.1.
