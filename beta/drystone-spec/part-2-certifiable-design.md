...
#### 4.5.1. Per-device authorship, per-device logical clock, and the user-principal-as-self-AS

Devices are **not** a shared key. Each device is a **client** (§5.2: one MLS leaf, one signature key, one
credential) — an independent signer with its own signature key — and an assertion is **authored and signed
per-device**: the envelope carries `author_device` (the signing identity the signature verifies against)
and `author_principal` (the user-principal the device is credentialed to). The device→principal binding is
established by a **credential**, not by key-sharing. This is the only model consistent with the underlying
group-key layer, which **forbids two devices of one user sharing a signature key in a group** and
represents each device as its own member — so "these devices are the same user" is necessarily an
**identity-layer credential policy**, not a key-layer fact. This is the mechanism behind the
governance-integrity spine (§5.2): because each client is a distinct member but governance counts
**principals**, a principal's clients are resolved to one identity before any quorum is counted, so more
devices never buy more weight. *(Group-key/MLS facts: confirm against the primary specification —
**[confirm before publish]**. The abstract group-key requirement this rests on — including the
forbid-shared-signature-key property as requirement K5 — is consolidated in **§10.2**, where MLS is
positioned as the reference realization rather than the requirement.)*

Because there is no central authority in a center-free system, the **user-principal key acts as its own
tiny certificate authority for its devices**: the user-principal signs each device's credential, and a
verifier accepts "this device is principal P" by validating that credential chain. Adding a device is the
deliberate, high-trust act (e.g. scanning a code) that the user-principal then attests — which is exactly
the trust binding of Part 1 §2.0 (the human act binds existing trust to a key; the credential records it).

A direct consequence for ordering, and a load-bearing instance of §2.0.1 (*time is not a fact*): the
**logical clock is per-device and strictly logical, never a wall-clock.** Each device advances only its own
`lamport` counter, so there are no cross-device collisions and no coordination is required; a wall-clock is
never consulted, because a wall-clock is an uncorroborable assertion and could not order what must
converge. A user-principal's stream is the **deterministic fold-time merge** across its devices' streams
(ordered by lamport, then a stable cryptographic tiebreak — never by time). `auth_assertions_by_device`
(the per-device causal index used for range-sync) keys on the device identity for exactly this reason.
...
- **Weight — how much a *peer* counts in governance.** **Flat: one per personhood-verified peer**,
  regardless of how many clients, devices, resources, or roles it holds. Weight is equal **by necessity,
  not by separate decree**: it *follows from* equal rights. If standing-to-participate is equal (the
  right), then standing-to-be-counted is equal (the weight) — the second is the governance image of the
  first. Attaches to the **peer** (§5.6). *("Personhood-verified" throughout means recognized as a distinct
  person by the group at its own contextual confidence (§5.6), not a global guarantee the protocol
  provides — the protocol guarantees provenance, the group judges personhood.)*

**The two inequalities** — legitimately different between peers:

- **Resource — what a *client/device* has.** Storage, uptime, reachability, a push token, a radio, "this
  box is willing to relay." A property of the **device**: **intrinsic**, **descriptive**, **expected to be
  unequal**, and **not delegable** — you cannot hand another device your RAM. A resource says what is
  *possible*, never what is *permitted* and never how much a peer *counts* (§5.4). *(This is the layer an
  earlier draft called "capability"; renamed because "capability" is Meadowcap's word for data access, and
  because "resource" names the device fact without inviting the false slide from "able to" to "entitled
  to.")*
...
- **Identity layer — principals and clients.** A **principal** is a role-holding entity identified by
  exactly one authenticatable identity (one key-lineage). A **client** is a single device — one MLS leaf,
  one signature key, one credential — and a principal is **realized by one or more clients**.

- **Governance layer — peers and weight.** A **peer** is the kind of principal that carries the rights
  floor and local canonical state, and is the source of one unit of governance weight.
...
- a **peer** — a principal that carries the rights floor and local canonical state (the common case: one
  human, one identity, possibly many devices);

- a **group** — a collective that can hold a role as a single principal (its identity model is an **open
  seam**, see below);

- a **meer (blind broker)** — a principal that holds a role (availability/relay) but has **no rights floor
  and no local history of its own**, and therefore **is not a peer** and carries **no governance weight**.
  *(The legacy label "mere-peer" is avoided in normative text precisely because a meer is not a peer; it is
  a principal that holds only a delegated role. "Meer" is the retained short name.)*

- a **delegate** — not a separate species but a **state**: a peer or group currently holding a role
  delegated by another principal (§5.5).

A **client** is a single device: one MLS **leaf**, one **signature key**, one **credential**, authenticated
as a **member** of a group via the **AS** (§10.2). A principal is **realized by one or more clients.** MLS
addresses clients; Drystone governance addresses principals.

> **The governance-integrity spine — identity, not client count.** Governance quorums and thresholds count
> **peers (by personhood-verified identity), never clients.** Five devices is one identity's worth of
> standing. This is what makes "you cannot buy your way to shifting the centre of gravity" structurally
> true: adding clients adds **resources**, never **rights** and never **weight**, because the count is
> over principals. The governance layer can discern the **lineage** of the key on a device that is a member
> of the MLS group, so a principal with one device and a principal with five are weighted identically.
> This is the property whose absence made early crypto-governance takeovers possible and painful to unwind;
> Drystone makes identity-not-resource the basis of weight by construction.

> **The keystone distinction — a peer is a provenance object; personhood is a social judgment.** These are
> different *kinds* of thing, not two views of one thing, and keeping them distinct is the identity-layer
> instance of the spec's founding provenance/utility split (§2.0).
>
> - A **peer** is **technically representable**: it is the root of a cryptographic-provenance lineage, a
>   thing the protocol can point at, verify signatures against, and count. The protocol delivers the peer
>   with certainty.
>
> - **Personhood** — whether that lineage corresponds to a distinct human — has **no technical
>   representation at all**, because it was never a fact the system holds. It is a judgment the *group*
>   makes (§5.6).
>
...
empty set (the meer of §5.2 authored nothing, so it satisfies the right vacuously). Blindness is the
*absence of a role conferring read*, never a restriction on a right.

> **Two open checks before the rights set hardens** (carried from Part 1 §2.4): the proven floor in this
...
> partly a membership-class **role**, and whether the §7 survivor/re-key path can strand `tenure`, both
> gate hardening the closed set into normative text. *design* (Appendix B). The Meadowcap distinction
> between **communal** namespaces (authority from owning a subspace key; horizontal, no apex) and
> **owned** namespaces (top-down single-owner control) is the nearest prior-art cut at this same seam —
> Drystone is communal for group governance and treats owned as the single-author-sub-content case
> (§5.10) — and should be studied before `share` is frozen (Appendix C). *(Meadowcap communal/owned
> semantics confirmed verbatim against the Willow Protocol spec; see §5.10.)*

### 5.4. Resources: device facilities, descriptive, not delegable

A **resource** is what a **client/device has** — a facility intrinsic to the hardware and its configured
intent. Resources are **descriptive** (they report what is possible), **unequal across devices**, and
**not delegable**: a device cannot hand another device its storage, uptime, or radio. A resource enables a
principal to *fulfil* a role (§5.5), or makes it *unsuited* to one, but holding a resource is never itself
a grant of authority and never adds governance weight.
...
- **Availability capacity.** The device can hold and serve a scope's *encrypted* objects, including
  buffering for offline members. Availability capacity does **not** imply read: a device with storage
  holds ciphertext it cannot decrypt unless its principal is separately granted a decryption role.
...
The pairing rule: a **role** (the governance authority, §5.5) is only useful to a principal whose
**client** has the **resource** to exercise it. Granting a read role to a device with no decryption
capacity is inert; granting an availability role to an always-off phone is inert. Roles and resources are
matched at the **PeerSet** layer (§5.5), which is exactly why a PeerSet bundles a role set *with an
expectation of resources*. The anti-capture consequence is in the words: a device may have more resources,
and that buys it no rights and no weight — only the ability to be *useful*, never to *count for more*.
...
- **Role — an in-group governance authority.** A scoped, attenuable authority to *act* in the group's
  ...
  deliberately leaves to the application** — RFC 9750 §3.5 states that MLS "does not itself enforce any
...
already in the log. This is why "it is a meer" is a *trustworthy* statement: the group enforces the pinned
composition.

> **Mutual exclusion fails loud, by design.** When a governance action would place two mutually-exclusive
> roles on one principal — e.g. a vote to grant an always-on availability meer a *decryption* role,
> stripping its blindness — the spec's preference is **not** a silent "forbidden" rejection but a **noisy
> hard-stop**: the action surfaces to the affected scope ("a restricted role combination was attempted;
> your communications may be at risk — fork without the principals who voted it?") and routes to human
> adjudication (§7.6). The reasoning: a quorum that votes to de-blind a meer everyone relied on as blind
> has *changed the trust dynamics of the group*, which is precisely the kind of standing contradiction
> that is a utility judgment, not a computation. *design — decided; the exclusion-as-loud-failure rung
> ties to §7.6.*

Worked example — the **meer** (an always-on blind helper), a PeerSet:

```
meer (a PeerSet) ::= floor                                  // full inherent rights, unchanged
                   + requires role { availability-serve }   // governance authority to hold/serve ciphertext
                   + expects  resource { availability-capacity, reachability }  // device facts that fulfil it
                   + forbids  role { decrypt, * issuing read-capability }        // mutually exclusive with read
                   + holds    capability { } // empty: a meer is granted no read/write data-access grant
```

A meer is a **principal** (it holds the availability-serve role) but **not a peer**: it has no rights
floor of its own beyond the vacuous read-your-own-history, and it carries **no governance weight**. It
satisfies read-your-own-history over an empty set (it serves ciphertext, holds no read capability, so it
holds no plaintext). Its blindness is the *enforced absence of any read capability or decrypt role*, not a
special restricted nature, and de-blinding it trips the mutual-exclusion hard-stop above. Delete the
PeerSet name and nothing about any principal's rights changes. *green-real (Tier-0 blind broker / running
blind meer); design (PeerSet drift-check and mutual-exclusion formalism).*
...
something. It attaches to the **peer**, and its default is **flat — one per personhood-verified peer**,
...
**Revocation/add authority is a threshold dial** (k-of-n, **counted by distinct personhood-verified peer,
...
> evaluated over **distinct personhood-verified peers**, so a single peer's multiple clients cannot
...
signers by distinct personhood-verified peer at the epoch it is set (solo genesis ⇒ `k_op = 1`; a scope
...
(the boundary between governing-the-group and owning-your-own-data, the `share`-vs-`tenure` seam of §5.3).
...
cannot, because **weight (§5.6) is anchored at the leaves: flat, one per personhood-verified peer, never
minted, only delegated.** However deep the composition, the total weight in any scope reduces to the count
of distinct personhood-verified peers at the bottom. A group-principal's weight in a parent scope is
...
> rather than a derived central credential). This is a Drystone construction question, not a gap in
> Meadowcap: Meadowcap's communal-namespace semantics are confirmed (below), but how Drystone *instantiates*
> a communal namespace as a governance-bearing group-principal, and rotates its key under membership churn,
> is the open design item. *(2)* **Cross-group** grants and references (one group-principal granting to or
> composing another across trust boundaries) are sketched here as the composition model but their wire
> encoding and the valuation-vs-composition edge (Part 1 §2.3) are `ENABLING`. Both carried to Appendix B.
>
...
> implies no ordering, this is acknowledged to be the **common case**, and so the system is, in their
> words, "basically trusting servers not to lie about the time," with damage mitigated only **after the
...
silent-false-resolution-risk) and **declines to pick the value** — that is a scope's call, and a
per-scope governed policy fact like any other. *design.*
...
> Cross-references: §4.5.1 (per-device authorship, device-as-member), §5.7 (revocation rotates epoch),
...
| K5 | Represent each device as its own member; **forbid** two devices of one user sharing a signature key in a group | this is what makes "same user" an identity-layer credential, not a key-layer fact (§4.5.1) |
...
- Cannot represent per-device membership (forces key-sharing across a user's devices), breaking K5 and the
  §4.5.1 model.
...
9420's design leans on *some* DS to order Commits. Drystone has **no DS** — it is center-free by
construction (§3.1), and ordering is the timestamp-free causal-cryptographic fold (§7.3.1) with forks
first-class (§7.6). This is precisely the gap K7 names and the reason MLS *as standardized* is necessary
but **not sufficient** for Drystone: the moment you remove the DS, concurrent commits fork and FS degrades
(Appendix A.1). So Drystone's relationship to MLS is:

- It **adopts** MLS's tree-based group-key core (K1–K6, K8) as the reference.

- It **diverges** from MLS's DS assumption — the DS is exactly the center Drystone refuses — and therefore
  must satisfy **K7** by other means: either a **FREEK-shaped puncturable-PRF mechanism**
...
> **A named, accepted cost — device-level metadata exposure (K5's price).** Representing each device as its
> own client/member (K5) is what makes governance count identities not clients (§5.2, §5.6), but it carries
> a real privacy cost the architecture documents acknowledge: because base MLS operates each device as a
> distinct client, other members can identify **which of a principal's devices sent each message** (and
> thus which device was in use), and can detect **when a principal adds or removes a device** — which RFC
> 9750 §8.2.4 ("Associating a User's Clients") notes may, for some applications, be an unacceptable breach
> of privacy. Per that section, the risk arises specifically when the clients' leaf nodes carry data that
> can correlate them, and is mitigated by careful handling of that leaf-node/credential data. Drystone's
> identity layer (the user-principal-as-CA, §4.5.1) is the separate user-level mechanism layered over the
> per-device clients, so the design is internally consistent — but the device-correlation metadata is a
...
| T2 | Provide end-to-end encryption such that intermediaries see ciphertext + routing metadata only | a relay/helper stays blind (§8); supports the blind-meer (§5.5) |
...
|---|---|---|---|
| Group key agreement | K1–K8 (§10.2), incl. FS-under-concurrency | **MLS** (RFC 9420/9750) core | wins on standardization + `log(N)` PCS; **diverges** from MLS's Delivery-Service ordering (Drystone has no DS), so K7 met via FREEK-shaped / Send-Groups / explicit-retention; AS role filled by user-principal-CA (§4.5.1) |
...
center (MLS's Delivery Service, any mandatory relay hop) that the standard realizations assume.**
...
- **The two open rights checks** (§5.3): is `share` fully a right or partly a membership-class **role**
  (or, if it is specifically a data-access claim, a Meadowcap **capability**); does the §7 survivor/re-key
  path strand `tenure`. Both gate freezing the four-rights closed set into normative text. Study the
  Meadowcap communal-vs-owned namespace distinction first (Appendix C).
...
- **What grounds a peer's authority — and what makes a right cost something to violate?** §3.1/§5.2 define
  ...
  **cryptographic-fact** authority (the protocol cannot route around a keyholder) — self-enforcing, but
  ...
  couples to the §5.8 exitability backstop and to the exit-as-remedy-for-capture framing above.
...
- **Willow data model + Meadowcap + range-based set reconciliation** (Earthstar/Willow project; Meyer's
  ...
  `share`-as-right-vs-role question and the model for the group-principal (§5.10). Willow uses BLAKE3
