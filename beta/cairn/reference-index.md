...
| iroh-tor | Pluggable iroh custom transport routing P2P traffic through Tor onion services; masks both peers' physical IP/location | learn↔ (metadata-floor hardening without leaving iroh) | community transport | github.com/n0-computer/iroh-tor | `[confirm]` per iroh-mobile FACTCHECK — real, not the `-transport` name, not on crates.io | substrate-prior-art |
| iroh-nym | Pluggable iroh custom transport over the Nym mixnet; packet mixing + artificial delay + cover traffic for who-talks-to-whom anonymity | learn↔ (metadata-floor hardening without leaving iroh) | community transport | github.com/n0-computer/iroh-nym | `[confirm]` per iroh-mobile FACTCHECK — real; sibling `iroh-webrtc-transport` flagged likely-hallucinated, not carried | substrate-prior-art |
...
| Marmot | The glue binding **Nostr** to **MLS** (RFC 9420); layers group E2EE onto the Nostr substrate, pairs with NIP-59 gift-wrap for metadata hiding | learn↔ (a live MLS-on-open-substrate cousin of Croft's exact thesis) | shipping (used by White Noise, Amethyst) | github.com/marmot-protocol/marmot | web-verified — confirmed real per opensocial FACTCHECK | adjacent-systems |
...
| Project | What it is | Rel. | Status / maturity | Locator | Marker | Covered in |
|---|---|---|---|---|---|---|
| Meadowcap | Willow's capability system; unforgeable grants with attenuation-by-subsetting | build-on / learn↔ (Track A candidate in the A11 capability-mechanism decision) | Final | willowprotocol.org/specs/meadowcap | verified | willow-meadowcap; object-capability (prior art). A11 decision itself recorded in `../drystone-spec/part-2-certifiable-design.md` Appendix A + §5.5, gated in `../DECISIONS.md` |
| Keyhive | Capability system; Track B candidate in the A11 capability-mechanism decision; one of the stacks Roomy tried | learn↔ (A11 Track B candidate) | pre-1.0 | repo as cited | dialogue-sourced 2026-06-24 | object-capability; adjacent-systems (prior art). A11 decision itself recorded in `../drystone-spec/part-2-certifiable-design.md` Appendix A + §5.5, gated in `../DECISIONS.md` |
...
| Nostr | Capture-resistant, credible-exit protocol; public-by-default at the base layer, with two private-group paths — relay-enforced (NIP-29 + NIP-42 auth) and true E2EE via Marmot/MLS + NIP-59 gift-wrap metadata hiding | learn↔ (capture-resistant; a serious MLS privacy answer, not merely leaky) | mature | nostr.com | landscape-grounded; NIPs + Marmot confirmed per opensocial FACTCHECK | adjacent-systems; atmospheric-web-and-aggregators |
...
| atproto relay (non-archival, post-Sync-v1.1) | Consumes PDS commit streams and re-emits the unified firehose; since Sync v1.1 (2025) it is **non-archival** — keeps a ~24h backfill window, not full repos; a validating full-network relay runs on ~2 vCPU / 12 GB / ~30 Mbps for ~$20–34/mo, Raspberry-Pi-capable (full-history backfill pulls from the PDS via `getRepo` or Tap) | build-on (self-hostable ingestion layer; grounds cheap-self-host / credible-exit) | current mechanics | docs.bsky.app/blog/relay-sync-updates; bnewbold "Full-Network Relay for $34 a Month" | `[confirm]` — CONFIRMED-current per the atproto architecture FACTCHECK; $ figures point-in-time, refresh before external use | atproto-selfhosting-appviews-and-bridges |
...
| Lexicon Community | Self-governed, Bluesky-independent lexicon namespace (`community.lexicon.*`: calendar, bookmarks, locations, reactions) held by a volunteer 7-member TSC; Discourse-first working-group chartering; the attestation work's nearest neighbor | learn↔, partner (governed-namespace hedge against domain loss; the engagement door) | active | github.com/lexicon-community/lexicon | verified (schema JSON); governance-body detail grounded-from-site, pending governance-repo pass | atproto-nsid-and-lexicon-mechanics; lexicon-community-governance |
...
| Session | Decentralized messenger; moved off the Signal Protocol to its own Session Protocol and dropped forward secrecy for years, then **restored PFS in Protocol V2 (Dec 2025) adding ML-KEM** | learn↔ (the lesson: never trade forward secrecy for decentralization — vindicates composing MLS) | shipping | getsession.org | verified per the messaging-solutions research (V2/ML-KEM restoration) | adjacent-systems |
| White Noise (+ Amethyst) | Live, privacy-focused **MLS group messenger on Nostr + Marmot** — metadata-hidden group chat on a decentralized substrate; Amethyst carries Marmot-compatible encrypted group channels | learn↔ (shipping cousin of Croft's exact thesis; still messaging, not a durable social data model + peer-symmetric governance) | shipping | parres-hq / White Noise | web-verified — confirmed real per opensocial FACTCHECK | adjacent-systems |
...
**Empirical proofs referenced (sibling Proofs/ repo, cited by name).** These are Croft's own experiments,
not ecosystem projects, so they are cited by name here rather than given directory rows.

- **Proof: public-roundtrip (atproto chain-of-custody), `Proofs/alpha/encrypted-local-first-atproto/`
  (PR #4).** Walked out live the DID → signing key → signed commit → MST root → record CID → record-bytes
  chain, verifiable with zero trust in the PDS/relay, and validated the labeler model (independently-signed,
  pull-only label assertions). Grounds the "cryptographic trust for free, zero semantic trust" boundary in
  `atproto-ecosystem.md`.
