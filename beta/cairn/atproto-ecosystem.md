...
**Attestation specs (the third-party-lexicon frontier).** A cluster of independent work is converging on signed attestations as an ATProto pattern, and it is pre-tooling: Bluesky states there are no hard blockers to third-party lexicons in the flagship app and the Lexicon Resolution protocol design is complete but SDK support is unimplemented, so fixtures and independent implementations carry outsized weight right now. Nick Gerakines (who also runs Smoke Signal) has published a formal attestation specification at badge.blue with two patterns — inline attestations embedding a signature in the record, and remote attestations storing proof in a separate record — using repository binding for replay prevention and CID content-addressing for integrity; his companion attested.network formalizes a three-party (payer/recipient/attester) proof-of-payments model, and Acudo applies the same line to organizer-signed RSVPs. The lexicon-community org's Discussion #8 ("Attestation Lexicon") is the governance thread, and its Polite Goshawk (Lenses) working group is a record-projection effort with no concrete format published yet. The Croft attestation lane's clean-room verifier was built from the badge.blue spec text alone and confirmed interoperable against Gerakines' own reference implementation; the one layer the published spec does not cover is attestation freshness/status, which is the Croft lane's stapling contribution (issuer-signed era tree heads, holder-stapled inclusion proofs, zero verifier callback). `Verified` (badge.blue spec + reference-impl interop, retrieved during RUN-LEX-01, 2026-07-19); details and the spec-ambiguity feedback in `../../alpha/experiments/lexicon-community/` and `../../alpha/experiments/attest-family/ENGAGE-LEX.md`.
