...
| `object-capability-and-decentralized-mls-prior-art.md` | Two prior-art strands: object-capability (Spritely Goblins/OCapN/CapTP — "designation is authorization", POLA, petnames, and the no-VC governance model) and decentralized-MLS siblings (DMLS/FREEK, draft-xue "TwoMLS" — the fork→FS cost and the "no production MLS is serverless-ordered" anchor). Cross-refs the A11 Track-A/B decision in `willow-meadowcap.md`. *(Recovery from ECOSYSTEM.md §2/§4.)* |
