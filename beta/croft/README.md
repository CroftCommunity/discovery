...
| `the-helper-tier-and-the-baseline-floor.md` | **Nothing server-side is load-bearing.** The honest **baseline floor** (two peers, same network, direct connection, manual join code — zero servers), the **helper contract** (optional · declared degradation · isolated trust surface · replaceable/self-hostable · reads direct), the **register** of optional helpers (AppView, login broker/BFF, followers-restriction, push, content-blind ingest, relay, deep-link resolver, delivery service) each with how it degrades back to the floor, the origin shape (single-origin core + isolated-subdomain + off-domain-untrusted, grounded in the WebKit storage-partitioning finding), and why it is carried forward from the protocol (peer-equality floor + durable-enablement + exitability). |
| `build-order-and-ponds-roadmap.md` | The **roadmap for the garden**: what to build and how it stacks. The ordering logic (resolver + substrate first, prove one pond before three, sequence by leverage); the Phase 0-5 build sequence (foundation → first pond → killer utility → the heart → breadth → optional assistant); the two parallel gates (resolver is tier-zero, sustainability sketched before the heart); the **per-pond build discipline** (determinism-first, verifiable-outcomes-via-replay, follow-chain-not-global comparison, a standing sustainment drill); and the **pond/pad catalog** stacking committed and candidate activities (games, split-the-check, presence/ritual, cards, aggregator, music-guessing, the account kernel) onto the sequence. Candidate/gated items point to `../OPEN-THREADS.md`. |
