# Platforms author the relational field: the structural argument

This document spells out the argument we have been building, in the order the claims depend on each other. It is written to be defensible, not maximal. Where a claim is sourced, the source is named inline. Where a claim is synthesis — ours, reasoned from the sourced parts — it is marked as such, because the strength of the whole depends on never letting an argued step pass as a found one.

The companion documents are the evidence brief (every claim with its primary source and epistemic tag) and the reference index (every source, sorted by quality). This document is the spine; those are the load-bearing members.

---

## The claim in one paragraph

Engagement-optimized platforms knowingly shape the content, emotional texture, and perceived social reality of users' relationships, in pursuit of an objective the users did not choose and that diverges from what users themselves say they want. They do this without meaningful disclosure. The mechanism they exploit is not new — a selection-and-effect feedback loop has always existed in human media behavior — but industrialization removed the bounds that used to keep that loop self-limiting and pointed it at a single external objective. The natural counterbalance to such a distortion is collective correction: people comparing notes and rebuilding shared reference. But the same optimization degrades the shared perception, trust, and attention that collective correction requires. The loop is therefore not merely unbalanced. It is structurally disadvantaged against its own correction, because the corrective faculty and the damaged faculty are the same faculty.

That paragraph has five planks. They do not all carry equal weight, and the rest of this document separates them so the strong ones are not dragged down by the contestable ones.

---

## Plank 1: The environment is engineered, not natural
...
Why it matters: "not disclosed by nature" is your phrase, and this is its grounding. The shaping force is not an emergent property of human nature; it is a designed artifact with an owner and an objective.

---

## Plank 2: The objective is not the user's

The optimization target is the platform's engagement/revenue proxy. The user did not choose it, and it demonstrably diverges from what users say they prefer. This is the cleanest, most defensible plank, and the argument should lead with it.
...
## Plank 3: The mechanism is pre-existing but was industrialized — and that is not trivial
...
This is the general pattern of detriment-at-scale, and naming the pattern is what makes "industrialized" non-trivial. A small-scale behavior is usually safe not because the act is benign but because it sits inside a web of counterforces that scale severs. A person eating to appetite is bounded by satiety; an engineered food environment optimized around craving removes satiety as the governor. Same structure: the harm is the removal of the bound, not the act.

Epistemic status: Slater is peer-reviewed-finding. The "industrialization removes the dampening term" reading is synthesis built directly on Slater's own dampening-term concept — close to the source, but argued, not quoted.

Seam (state it so the metaphor is not over-trusted): not every scaled mechanism that sheds its small-scale bounds becomes harmful. Vaccination, standardized measurement, and error-correcting codes all scale while preserving or adding correction. "Scale removes counterbalance" is therefore not a law; it is a tendency that holds specifically when the scaling is driven by a single misaligned optimizer and fails when the scaling preserves correction. This distinction is your defense against the "you are just anti-scale / anti-technology" rebuttal. The problem is not scale. The problem is scale under a single objective that has no interest in preserving the system's correction mechanism.

---

## Plank 4: The system is not reliably self-correcting
...
## Plank 5: The corrective faculty and the damaged faculty are the same (the reflexive claim — this is synthesis)

This is the sharp point of the whole argument, and it is the part not stated in any single source. It is built from sourced parts; it is labeled as argument throughout.

The setup: the natural counterbalance to a distorted social-information environment is collective sense-making. People compare notes, correct each other, build shared reference, and slowly converge on what is true and what they value. That social process is the negative feedback loop at the scale of a group. It is, functionally, the governor that Plank 4 says the engineered system lacks.

The claim: collective sense-making runs on exactly the faculties that engagement optimization degrades. Four of them, each with a sourced degradation:

Shared reference. Collective correction requires a common picture of what others think. Engagement-amplified content produces false polarization — people perceive the out-group as more extreme and more homogeneous than it is (Lees & Cikara, 2020, *Nature Human Behaviour*, on inaccurate group meta-perceptions; the broader false-polarization construct). Fragmented shared reference is a degraded input to correction.

Good-faith disagreement. Correction requires productive disagreement, but the feed rewards the hostile version of it (your Milli et al. 2025 grounding on emotional/partisan amplification; and the engagement-vs-quality divergence in Zuckerberg's 2018 note).
...
## The honest qualifier: disadvantaged, not destroyed

The reflexive claim is strong, and the temptation is to state it as "rebalancing is currently impossible." That overstates it, and the overstatement is both less true and weaker.

Correction still happens. Regulators acted. Researchers published the very studies in these documents. Haugen leaked. YouTube changed its recommender in 2019 (and the trace studies — Chen et al. 2023, Hosseinmardi et al. 2021/2024 — suggest that change mattered). People leave platforms.

So the defensible claim is asymmetry, not impossibility: the corrective forces are slower, costlier, and structurally downstream of the thing they correct, while the optimization is fast, cheap, continuous, and operating on the correction mechanism itself. "Currently not possible" → "systematically outpaced and undermined." The weaker-sounding version is much harder to knock down, and it is the one the evidence supports.

The trace studies actually help here rather than hurt. They show users retain real agency — extreme consumption tracks pre-existing demand, the algorithm did not override preference for the average person. A critic uses this to say "people get what they want; that is autonomy, not capture." The answer is the choice-architecture distinction: shaping defaults, salience, and what is adjacent is influence over the conditions of choice even when it does not override the choice itself. That is a real and defensible position, but it is a normative argument about what counts as autonomy, not a settled empirical fact. Frame it as the philosophical claim it is. And note that it makes the indictment more credible, not less: the corrective faculty is not dead, it is operating in an environment tilted against its exercise. A disadvantaged-but-living corrective is a more damning and more believable claim than capture.

---

## What the argument needs that it does not yet have

Stated plainly so the gaps are visible rather than hidden.

The reflexivity claim is sourced (Milano et al. 2021, NMI; Milano & Prunkl 2024, Phil Studies), and the ranking-specific extension is now also sourced (Burton, Herzog & Lorenz-Spreen 2024/2026). The only remaining synthesis in Plank 5 is the four-faculty decomposition, which is a structuring of sourced degradations rather than a new empirical claim. This plank is, for practical purposes, grounded.

A primary for the "break-glass" count. The claim that Meta keeps and pulls emergency demotion levers is itself evidence that the company believes its normal ranking amplifies harm. Currently sourced to a critique, not a Meta document.

The teen mental-health internal research is now pulled to primary (gap 13 in the brief) and woven into Plank 2 as the concrete human-scale instance, with the mandatory framing caveat (one in three of girls reporting body-image issues, not all girls; self-report, non-causal). The post-2021 disclosures are also pulled: Project Mercury (gap 14), the internal deactivation study alleged in a Nov 2025 litigation filing, is the closest thing to internal causal evidence — used in Plank 2 with its three status flags (litigation not adjudication; documents not public / motion to strike; Meta's expectation-effects rebuttal). A hearing was set for Jan 26, 2026, at the edge of what this work can verify; check the live docket before leaning on it. The Tristan Harris primary remains not-yet-pulled. Béjar's 2023 Senate testimony and the broader set of ~35 internal studies (per the metasinternalresearch.org compilation) are partially surfaced but most individual studies are not pulled to primary here.

---

## One-line version, for when you need it

They rebuilt the room we talk to each other in, optimized it for a goal that is not ours, removed the friction that used to keep it honest, and in doing so wore down the very capacity we would use to notice and push back — so the correction is not impossible, just always a step behind by design.
