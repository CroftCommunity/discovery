...
| `platform-extraction-and-captured-labor.md` | The extraction strand of the harm case (T35), distinct from the relational-field/attention strand: captured community labor (unpaid moderators as platform value), enshittification read as an indictment (funded phase then extraction phase), and data opacity / lock-in as the mechanism that makes exit costly. Counter-evidence at full strength (CONTESTED markers; the non-extractive exceptions); private-platform financials [UNVERIFIED]. The descriptive dominance/lifecycle map lives in `../fenced/`. | indictment, honest-other-side |
